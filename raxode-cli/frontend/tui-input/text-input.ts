...
import { isTerminalMouseInput } from "./mouse.js";
...
const COMBINING_MARK_RANGES: readonly [number, number][] = [
  [0x0300, 0x036f],
  [0x1ab0, 0x1aff],
  [0x1dc0, 0x1dff],
  [0x20d0, 0x20ff],
...
const VARIATION_SELECTOR_RANGES: readonly [number, number][] = [
  [0xfe00, 0xfe0f],
  [0xe0100, 0xe01ef],
];
...
function codePointInRanges(codePoint: number, ranges: readonly [number, number][]): boolean {
  return ranges.some(([start, end]) => codePoint >= start && codePoint <= end);
}

function isFastSingleCodeUnitGrapheme(value: string, index: number): boolean {
  if (index < 0 || index >= value.length) {
    return false;
  }
  const codeUnit = value.charCodeAt(index);
  if (
    (codeUnit >= HIGH_SURROGATE_MIN && codeUnit <= HIGH_SURROGATE_MAX)
    || (codeUnit >= LOW_SURROGATE_MIN && codeUnit <= LOW_SURROGATE_MAX)
    || codeUnit === ZERO_WIDTH_JOINER
    || codePointInRanges(codeUnit, COMBINING_MARK_RANGES)
    || codePointInRanges(codeUnit, VARIATION_SELECTOR_RANGES)
  ) {
    return false;
  }
  const previousCodeUnit = value.charCodeAt(index - 1);
  const nextCodeUnit = value.charCodeAt(index + 1);
  return previousCodeUnit !== ZERO_WIDTH_JOINER
    && nextCodeUnit !== ZERO_WIDTH_JOINER
    && !codePointInRanges(nextCodeUnit, COMBINING_MARK_RANGES)
    && !codePointInRanges(nextCodeUnit, VARIATION_SELECTOR_RANGES);
}
...
function previousGraphemeOffset(value: string, offset: number): number {
  ...
  const previousCodeUnit = value.charCodeAt(normalizedOffset - 1);
  if (
    normalizedOffset > 0
    && previousCodeUnit >= ASCII_PRINTABLE_MIN
    && previousCodeUnit <= ASCII_PRINTABLE_MAX
  ) {
    return normalizedOffset - 1;
  }
  if (isFastSingleCodeUnitGrapheme(value, normalizedOffset - 1)) {
    return normalizedOffset - 1;
  }
...
function nextGraphemeOffset(value: string, offset: number): number {
  const normalizedOffset = clampOffset(value, offset);
  const nextCodeUnit = value.charCodeAt(normalizedOffset);
  if (
    normalizedOffset < value.length
    && nextCodeUnit >= ASCII_PRINTABLE_MIN
    && nextCodeUnit <= ASCII_PRINTABLE_MAX
  ) {
    return normalizedOffset + 1;
  }
  if (isFastSingleCodeUnitGrapheme(value, normalizedOffset)) {
    return normalizedOffset + 1;
  }
  const breaks = graphemeBreaks(value);
...
export function applyTuiTextInputKey(
  ...
  if (isTerminalMouseInput(inputText)) {
    return { nextState: state, submit: false, handled: true };
  }
