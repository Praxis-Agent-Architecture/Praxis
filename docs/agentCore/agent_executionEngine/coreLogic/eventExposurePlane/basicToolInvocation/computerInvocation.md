...
> 对应源码：`src/executionEngine/coreLogic/eventExposurePlane/basicToolInvocation/computerInvocation.ts`

## 1. 文件位置

- 所属顶层模块：执行引擎（`agent_executionEngine`）。
- 所属路径：`agent_executionEngine/coreLogic/eventExposurePlane/basicToolInvocation`。
- 当前文件：`computerInvocation.ts`。
...
- 原始文件名：`computerInvocation.ts`。
- 命名片段：`computer` / `Invocation`。
