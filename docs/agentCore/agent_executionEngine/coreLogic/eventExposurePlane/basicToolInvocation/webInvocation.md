...
> 对应源码：`src/executionEngine/coreLogic/eventExposurePlane/basicToolInvocation/webInvocation.ts`

## 1. 文件位置

- 所属顶层模块：执行引擎（`agent_executionEngine`）。
- 所属路径：`agent_executionEngine/coreLogic/eventExposurePlane/basicToolInvocation`。
- 当前文件：`webInvocation.ts`。
...
这个文件的核心不是“占一个目录位置”，而是要在当前路径上形成一个可实现、可测试、可被 runtime 或相邻模块调用的窄能力点。它应该围绕“暴露 web 基础工具调用事件”建立清晰的输入、输出、错误和治理边界。

## 2.1 文件名语义拆解

- 原始文件名：`webInvocation.ts`。
- 命名片段：`web` / `Invocation`。
...
- 执行“暴露 web 基础工具调用事件”所需的事件来源、会话标识、调用上下文、治理上下文和订阅范围。

输入边界必须窄：只接收完成本文件职责所需的材料，不把相邻模块的大对象整包吞进来。

## 7. 输出边界

- web.search 或 web.fetch 调用后形成的标准化结果。
...
- 实现一个最小纯函数或薄类壳，能完成“暴露web 基础工具调用事件”的可测路径。
...
- 验证该文件确实只完成“暴露web 基础工具调用事件”，没有越界承担相邻模块职责。
