---
description: prepare an audio understanding request while leaving decoding, transcription, and provider lowering to runtime.
...
Use `omni.listenAudio` when an agent needs to prepare an audio understanding request while leaving decoding, transcription, and provider lowering to runtime. This baseTool is a contract and handler surface; it does not read media bytes, run codecs, upload files, poll jobs, choose a provider model, or lower provider-specific request bodies.
