---
description: "Read blame ownership for a bounded file line range."
argument-hint: '{"target":{"repositoryPath":"/repo/project","filePath":"src/index.ts","range":{"startLine":1,"endLine":5}},"context":{"dryRun":false,"guard":{"allowed":true,"accepted":true},"allowedRepositoryRoots":["/repo"],"grantedPermissions":["git:read","filesystem:read"]}}'
---
...
Use `git.traceLineOwnership` to read blame ownership for a bounded file line range. It is a fixed-action gitBase tool exposed through the unified `BaseToolHandler.invoke()` surface. The model must call this narrow tool rather than `shell.commandExecution`, `git.execute`, or a made-up `gitBase.*` tool id.
...
- `blame --line-porcelain -L <start>,<end> [revision] -- <filePath>`
...
- Do not bypass `createBaseToolRegistry().lookupHandler("git.traceLineOwnership")` and `handler.invoke(...)` in integration tests.
