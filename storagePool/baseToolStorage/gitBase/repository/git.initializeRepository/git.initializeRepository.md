---
description: "Initialize Git metadata in an approved directory."
argument-hint: '{"target":{"repositoryPath":"/repo/new-project","initialBranch":"main"},"context":{"dryRun":false,"guard":{"allowed":true,"accepted":true},"allowedRepositoryRoots":["/repo"],"grantedPermissions":["git:write","filesystem:write"]}}'
---
...
Use `git.initializeRepository` to initialize Git metadata in an approved directory. It is a fixed-action gitBase tool exposed through the unified `BaseToolHandler.invoke()` surface. The model must call this narrow tool rather than `shell.commandExecution`, `git.execute`, or a made-up `gitBase.*` tool id.
...
- `target.initialBranch`.
- `target.bare`.
- `target.separateGitDir`.
- `timeoutMs`.
...
- `init [--initial-branch <branch>] [--bare] [--separate-git-dir <path>]`
...
- Do not bypass `createBaseToolRegistry().lookupHandler("git.initializeRepository")` and `handler.invoke(...)` in integration tests.
