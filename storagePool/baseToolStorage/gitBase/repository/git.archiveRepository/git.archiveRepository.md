---
description: "Write an archive from a ref through fixed git archive semantics."
argument-hint: '{"target":{"repositoryPath":"/repo/project","outputPath":"/repo/project.tar","ref":"HEAD","format":"tar","pathspecs":["src"]},"context":{"dryRun":false,"guard":{"allowed":true,"accepted":true},"allowedRepositoryRoots":["/repo"],"grantedPermissions":["git:read","filesystem:write"]}}'
---
...
Use `git.archiveRepository` to write an archive from a ref through fixed git archive semantics. It is a fixed-action gitBase tool exposed through the unified `BaseToolHandler.invoke()` surface. The model must call this narrow tool rather than `shell.commandExecution`, `git.execute`, or a made-up `gitBase.*` tool id.
...
- `archive [--format tar|zip] [--output <outputPath>] <ref> <pathspecs>`
...
- Do not bypass `createBaseToolRegistry().lookupHandler("git.archiveRepository")` and `handler.invoke(...)` in integration tests.
