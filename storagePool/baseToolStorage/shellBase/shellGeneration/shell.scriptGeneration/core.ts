...
function invocationId(context: ShellScriptGenerationContext | undefined): string {
  return typeof context?.invocationId === "string" && context.invocationId.trim().length > 0
    ...
    : "shell.scriptGeneration:dry-run";
}
...
function normalizeTarget(
  ...
  if (target?.scriptName !== undefined && typeof target.scriptName !== "string") {
    return failure("INVALID_SCRIPT_NAME", "shell.scriptGeneration scriptName must be a string when provided", "input", context);
  }

  const scriptName = typeof target?.scriptName === "string" ? target.scriptName.trim() || undefined : undefined;
  ...
  if (
    ...
    (!Array.isArray(target.commands) || target.commands.some((command) => typeof command !== "string"))
  ) {
    return failure("INVALID_COMMAND", "shell.scriptGeneration commands must be an array of strings", "input", context);
  }
  ...
  if (target?.workingDirectory !== undefined && typeof target.workingDirectory !== "string") {
    return failure(
      "INVALID_WORKING_DIRECTORY",
      "shell.scriptGeneration workingDirectory must be a string when provided",
      "input",
      context,
    );
  }

  const workingDirectory = typeof target?.workingDirectory === "string" ? target.workingDirectory.trim() || undefined : undefined;
  return {
    scriptName,
    shell,
    workingDirectory,
...
export function generateShellScriptPlan(
  ...
  if (!isRecord(request)) {
    return failure("MISSING_COMMANDS", "shell.scriptGeneration requires at least one command", "input", undefined);
  }
