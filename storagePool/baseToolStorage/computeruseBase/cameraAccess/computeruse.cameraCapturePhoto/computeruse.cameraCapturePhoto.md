---
description: Capture a camera photo artifact through the runtime-owned computer-use media port.
argument-hint: "{ target?: { cameraId: string, purpose: string, outputFormat?: 'image/jpeg'|'image/png'|'image/webp', permissionLeaseId?: string }, context?: { dryRun?: boolean, guard?: { accepted?: boolean, allowed?: boolean } } }"
---
...
Use this tool when runtime/TAP has already decided that the next primitive action is to capture a still image from a camera device and return a governed artifact reference.

This is a low-level camera photo capability. It is not a permission request tool, device selection tool, video recorder, face-recognition tool, omni routing step, browser controller, or fallback strategy.

## Call Shape

Call through `createBaseToolRegistry().lookupHandler("computeruse.cameraCapturePhoto").handler.invoke(...)` with a full `BaseToolInvokeRequest` envelope:

```ts
await handler.invoke({
  toolCallId: "camera-photo-1",
  runtimeId: "runtime-1",
  sessionId: "session-1",
  input: {
    target: {
      cameraId: "default-camera",
      purpose: "capture a session-scoped verification photo",
...
- `target.purpose` or top-level `purpose`: why the camera photo is being captured.
...
- For real execution, runtime must provide `BaseToolExecutorPort.computeruse.captureCameraPhoto` through `executor.computeruse`.

## Optional Inputs

- `target.deviceId`: accepted as a compatibility alias for `target.cameraId`.
- `target.outputFormat` or top-level `outputFormat`: `image/jpeg`, `image/png`, or `image/webp`; defaults to `image/jpeg`.
...
`core.ts` owns unknown-JSON validation, camera id normalization, purpose/output format validation, scope checks, dry-run planning, guard checks, provider-missing behavior, provider-failure mapping, and the public result envelope.

Dry-run is the default. Dry-run returns a metadata-only capture plan and never calls a provider.

Real execution dispatches only through `BaseToolExecutorPort.computeruse.captureCameraPhoto` with `cameraId`, `purpose`, `outputFormat`, optional permission lease metadata, and runtime invocation metadata. If `executor.computeruse` or `captureCameraPhoto` is absent, the tool returns `PROVIDER_UNAVAILABLE`.

Runtime owns camera frame acquisition, permission lease enforcement, device policy, artifact storage, MIME typing, privacy cleanup, platform adapters, and any OS prompts. The baseTool only declares the contract and normalizes the runtime artifact result.
...
- `output.kind`: `agentCore.basicTool.computeruse.cameraCapturePhoto`.
- `output.dispatch`: `dry-run` or `runtime-computeruse`.
- `output.target`: normalized `cameraId`, `purpose`, `outputFormat`, and optional `permissionLeaseId`.
- `output.providerCalled`: whether the runtime port was invoked.
- `output.runtimeEntry.port`: `BaseToolExecutorPort.computeruse.captureCameraPhoto`.
- `output.runtimeEntry.baseToolOwnsTapStrategy`: always `false`.
- `output.artifactEnvelope`: dry-run metadata, or real `captured`, `artifactId`, and `mimeType`.
...
Stable public errors include `INVALID_REQUEST`, `INVALID_CONTEXT`, `INVALID_TARGET`, `MISSING_RUNTIME_ID`, `MISSING_CAMERA_ID`, `MISSING_PURPOSE`, `INVALID_CAMERA_ID`, `INVALID_PURPOSE`, `INVALID_OUTPUT_FORMAT`, `INVALID_PERMISSION_LEASE`, `SCOPE_DENIED`, `CONTRACT_REJECTED`, `GOVERNANCE_REJECTED`, `PROVIDER_UNAVAILABLE`, and `PROVIDER_FAILURE`.
...
- Do not use this tool to request camera permission, select a camera, record video, or run face recognition. Those are separate cameraAccess primitives and runtime/vision-provider concerns.
