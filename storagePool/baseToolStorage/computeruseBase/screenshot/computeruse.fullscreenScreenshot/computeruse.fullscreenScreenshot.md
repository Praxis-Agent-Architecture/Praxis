---
description: Capture a fullscreen screenshot through the runtime-owned computer-use port.
argument-hint: "{ purpose: string, target?: { displayId?: string, outputFormat?: 'image/png'|'image/jpeg'|'image/webp' }, context?: { dryRun?: boolean, guard?: { accepted?: boolean, allowed?: boolean } } }"
---
...
Use this tool when runtime/TAP has already decided that the next primitive action is to request a fullscreen screenshot artifact from the active computer-use surface.
...
Call through `createBaseToolRegistry().lookupHandler("computeruse.fullscreenScreenshot").handler.invoke(...)` with a full `BaseToolInvokeRequest` envelope:
...
`core.ts` owns unknown-JSON validation, target normalization, output-format validation, dry-run planning, guard checks, scope checks, provider-missing behavior, provider-failure mapping, and the public result envelope.

Dry-run is the default. Dry-run returns a metadata-only capture plan and never calls a provider.

Real execution dispatches only through `BaseToolExecutorPort.computeruse.captureScreenshot`. If `executor.computeruse` or `captureScreenshot` is absent, the tool returns `PROVIDER_UNAVAILABLE`.
...
- `output.kind`: `agentCore.basicTool.computeruse.fullscreenScreenshot`.
- `output.dispatch`: `dry-run` or `runtime-computeruse`.
- `output.target.displayId` and `output.target.outputFormat`.
...
Stable public errors include `INVALID_REQUEST`, `INVALID_CONTEXT`, `INVALID_TARGET`, `MISSING_RUNTIME_ID`, `MISSING_PURPOSE`, `INVALID_OUTPUT_FORMAT`, `SCOPE_DENIED`, `CONTRACT_REJECTED`, `GOVERNANCE_REJECTED`, `PROVIDER_UNAVAILABLE`, and `PROVIDER_FAILURE`.
