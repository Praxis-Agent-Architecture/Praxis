...
function buildLedger() {
  ...
  for (const sourceAbs of sources) {
    ...
    const fileTask = {
      ...
      bigSpecId: big.id,
      status: "pending",
      sourcePath: `src/agentCore/${rel}`,
      docPath: `docs/agentCore/${docRel}`,
      testPath: `test/agentCore/${testRel}`,
    };
...
function commandPreflight() {
  ...
  const dirty = shell("git", ["status", "--short", "--", "."]);
  checks.push({
    name: "foundation committed for worktree",
    ok: dirty.stdout.trim().length === 0,
    detail: dirty.stdout.trim() ? "dirty/untracked Praxis framework files exist" : "clean",
  });
...
function ensureWorktreeDependencies(group, execute) {
  ...
  const worktreeNodeModules = path.join(group.worktreePath, "node_modules");
...
function mainDirtyStatus({ ignoreLedger = false } = {}) {
  ...
  return dirty
    .split("\n")
    .filter((line) => !line.endsWith("tasks/ledger.json"))
    .join("\n")
    .trim();
}
