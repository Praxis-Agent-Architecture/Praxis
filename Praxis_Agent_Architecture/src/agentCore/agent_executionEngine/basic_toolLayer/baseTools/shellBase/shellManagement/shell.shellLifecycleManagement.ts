...
export type ShellLifecycleManagementBoundary = "input" | "permission" | "scope" | "approval" | "resource" | "contract";
...
function invocationId(context: ShellLifecycleManagementContext | undefined): string {
  return context?.invocationId?.trim() || "shell.shellLifecycleManagement:dry-run";
}
...
function ensureApproval(
  ...
  if (context?.approval?.accepted === false) {
    return failure(
      "APPROVAL_REJECTED",
      context.approval.reason ?? "shell.shellLifecycleManagement approval was rejected by TAP governance",
      "approval",
      context,
      sessionId,
      workingDirectory,
  ...
  return failure(
    "APPROVAL_REQUIRED",
    "shell.shellLifecycleManagement close action requires TAP approval",
    "approval",
    context,
    sessionId,
    workingDirectory,
...
function normalizeTarget(
  ...
  if (!lifecycleActions.has(action)) {
    return failure("INVALID_ACTION", "shell.shellLifecycleManagement action is not supported", "input", context, target?.sessionId);
  }

  const requestedSessionId = target?.sessionId?.trim() || undefined;
  if (action !== "create" && requestedSessionId === undefined) {
  ...
  const sessionId = plannedSessionId(action, requestedSessionId, context);
...
export function planShellLifecycleManagement(
  ...
  const approvalFailure = ensureApproval(target.action, request.context, target.sessionId, target.workingDirectory);
