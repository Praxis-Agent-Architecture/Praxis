---
description: Governed shell process spawning baseTool implementation.
...
Use `shell.processSpawning` when the runtime needs a stable shell process-control primitive. The baseTool defines the JSON shape, dry-run envelope, public-safe errors, audit events, and provider dispatch boundary.

## Call Shape

```ts
await executeShellProcessSpawning({
