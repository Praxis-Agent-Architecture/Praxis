---
description: Governed detached shell execution launch baseTool implementation.
...
Use `shell.detachedExecution` when the runtime needs a stable shell process-control primitive. The baseTool defines the JSON shape, dry-run envelope, public-safe errors, audit events, and provider dispatch boundary.

## Call Shape

```ts
await executeShellDetachedExecution({
...
```ts
const result = await executeShellDetachedExecution({
  target: { command: "pwd" },
