...
export type ShellProcessManagementBoundary = "input" | "permission" | "scope" | "approval" | "resource" | "contract";
...
function invocationId(context: ShellProcessManagementContext | undefined): string {
  return context?.invocationId?.trim() || "shell.shellProcessManagement:dry-run";
}
...
function ensureApproval(
  ...
  if (context?.approval?.accepted === false) {
    return failure(
      "APPROVAL_REJECTED",
      context.approval.reason ?? "shell.shellProcessManagement approval was rejected by TAP governance",
      "approval",
      context,
      sessionId,
      processId,
  ...
  return failure(
    "APPROVAL_REQUIRED",
    "shell.shellProcessManagement destructive process changes require TAP approval",
    "approval",
    context,
    sessionId,
    processId,
...
function normalizeTarget(
  ...
  if (!processActions.has(action)) {
    return failure("INVALID_ACTION", "shell.shellProcessManagement action is not supported", "input", context, target?.sessionId);
  }
  ...
  if (action === "signal" && signal === undefined) {
  ...
  if (signal !== undefined && !processSignals.has(signal)) {
  ...
  if (action === "prioritize" && (typeof priority !== "number" || !Number.isInteger(priority) || priority < -20 || priority > 19)) {
    return failure(
      "INVALID_PRIORITY",
  ...
  const requiresTapApproval = approvalRequired(action, signal);
  ...
  return {
    ...
    dryRun: true,
    processChangeBlocked: true,
    unsafeSideEffects: action !== "inspect",
    resultEnvelope: {
      planned: true,
      observedStatus: action === "inspect" ? "unknown" : undefined,
    },
  };
}
...
export function planShellProcessManagement(
  ...
  const approvalFailure = ensureApproval(target.requiresTapApproval, request.context, target.sessionId, target.processId);
