...
argument-hint: "{ target: { inputPath | inputRef, outputPath | outputRef, targetFormat?, maxBytes? }, context?: { dryRun?, guard? } }"
---
...
Use `omni.imageCompressor` when an agent needs to prepare an image compression request for the runtime-owned omni media pipeline. This baseTool is a contract and handler surface; it does not read media bytes, run codecs, upload files, poll jobs, choose a provider model, or lower provider-specific request bodies.
...
Provide `target.inputPath` or `target.inputRef`, plus `target.outputPath` or `target.outputRef`.

## Optional Inputs

- `target.targetFormat`: requested output format or MIME-style runtime hint.
- `target.maxBytes`: runtime compression or size budget hint when relevant.
...
await handler.invoke({
  ...
  sessionId: "session-1",
  input: {
    target: { inputPath: "/workspace/media/source.png", outputPath: "/workspace/output/result.webp", targetFormat: "webp", maxBytes: 500000 },
    context: {
      dryRun: false,
      guard: { accepted: true, allowed: true },
      grantedPermissions: ["omni:image:read", "omni:image:write"]
    }
  },
  executor
});
