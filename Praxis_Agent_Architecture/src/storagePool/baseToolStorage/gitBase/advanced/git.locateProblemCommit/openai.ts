import type { GitLocateProblemCommitProviderPractice } from "./dependencies.js";
import { createHostExecutorGitLocateProblemCommitProvider } from "./dependencies.js";

export const openaiGitLocateProblemCommitPractice: GitLocateProblemCommitProviderPractice = {
  providerName: "openai",
  source: {
    kind: "cli",
    label: "Codex git history practice through sandboxed runtime executor ownership",
  },
  directCliSupport: true,
  sideEffectPolicy: "runtime-governed",
  notes: [
    "Codex-style practice keeps host process execution behind a runtime executor and sandbox policy.",
    "Praxis stores the locate-problem-commit contract in gitBase while runtime owns the real git process.",
  ],
  createProvider: ({ provider, executor }) => provider ?? createHostExecutorGitLocateProblemCommitProvider(executor),
};
