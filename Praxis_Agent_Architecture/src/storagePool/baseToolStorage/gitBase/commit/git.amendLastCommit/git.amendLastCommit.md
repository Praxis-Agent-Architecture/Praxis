...
> 对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_executionEngine/basic_toolLayer/baseTools/gitBase/commit/git.amendLastCommit.ts`

## 1. 文件位置

- 所属顶层模块：执行引擎（`agent_executionEngine`）。
- 所属路径：`agent_executionEngine/basic_toolLayer/baseTools/gitBase/commit`。
- 当前文件：`git.amendLastCommit.ts`。
- storage 实现：`src/storagePool/baseToolStorage/gitBase/commit/git.amendLastCommit/`。

## 2. 文件职责

提供 Git 基础工具 / 提交操作 中的“修订最后一次提交”基础能力原语。

它把修订最后一次提交做成固定动作工具：storage 负责 commit message / no-edit 校验、固定 argv、风险和结果解析，runtime 只通过 `BaseToolExecutorPort.git.runGit` 执行 `{ repositoryPath, args, timeoutMs }`。这里是基础工具原语，不是 TAP 的最终高级工具库；TAP 仍负责更上层审批、组合和专业工具。

## 2.1 文件名语义拆解

- 原始文件名：`git.amendLastCommit.ts`。
- 命名片段：`git` / `amend` / `Last` / `Commit`。
- 工程含义：这是 `gitBase` 下 `commit` 分组里的 `amendLastCommit` 基础工具原语。
- 真实动作：固定为 `git commit --amend`，不提供任意 `git.execute`。
...
- 通过统一 `BaseToolHandler.invoke()` 暴露 `git.amendLastCommit`。
- 支持固定 `git commit --amend [--all] [--reset-author] (--no-edit | -m <commitMessage>)`。
- 返回 `runtimeEntry`、`risk`、`gitArgs`、`commandPreview`、`providerCalled`、`executionBlocked` 和 `resultEnvelope`。
- 保留 `planGitLastCommitAmend()` 兼容旧调用方。

## 6. 输入边界

- 输入必须是工具调用 JSON，主要字段是 `target.repositoryPath`、`target.commitMessage` 或 `target.noEdit`、`includeAllTracked`、`resetAuthor`。
...
- 真实 amend 必须设置 `dryRun:false` 和 affirmative guard。

## 7. 输出边界

- 输出是可给 runtime inspection 和上层模型看的标准工具结果信封。
- 不泄漏 provider 栈、私有路径或 `.git` 内部细节。
- `resultEnvelope` 至少描述 commit message / no-edit、operation hint、commit hash、branch、subject、files changed 和 commitAmended 状态。

## 8. 错误边界

- 参数错误返回 `INVALID_ARGUMENT`、`MISSING_REPOSITORY_PATH` 或 `MISSING_REQUIRED_FIELD`。
...
- `createBaseToolRegistry().lookupHandler("git.amendLastCommit")`。
- `scripts/agentCore_Agent_Test/agentcore_tool_lab.ts` 的 mounted gitBase runner。
- 后续 TAP 高级工具系统可以把它组合成“暂存 -> 修订提交 -> 检查状态”的完整工作流。

## 11. 不应该做什么

- 不要提供任意 `git.execute`。
- 不要改用 `shell.commandExecution` 执行 amend。
- 不要把 create/cherry-pick/revert/reset/merge/rebase/push 等动作塞进本工具。
- 不要让模型传入路径后自动成为 allowed repository root。

## 12. 最小实现建议

- entry 文件保持薄 re-export。
- storage `core.ts` 负责 JSON 验证、commit message / no-edit 校验、固定 argv、dry-run、governance、provider error mapping 和 parser。
...
- fake runtime executor 收到固定 argv，例如 `commit --amend --all -m Update last commit`。
