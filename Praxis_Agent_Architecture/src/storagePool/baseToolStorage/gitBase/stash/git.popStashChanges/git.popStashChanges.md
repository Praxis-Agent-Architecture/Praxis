---
description: "Apply a stash entry and drop it on success."
argument-hint: '{"target":{"repositoryPath":"/repo/project","stashRef":"stash@{0}","reinstateIndex":true},"context":{"dryRun":false,"guard":{"allowed":true,"accepted":true},"allowedRepositoryRoots":["/repo"],"grantedPermissions":["git:read","git:write","filesystem:read","filesystem:write"]}}'
---
...
Use `git.popStashChanges` to apply a stash entry and drop it on success. It is a fixed-action gitBase tool exposed through the unified `BaseToolHandler.invoke()` surface. The model must call this narrow tool rather than `shell.commandExecution`, `git.execute`, or a made-up `gitBase.*` tool id.
...
- `target.stashRef`: defaults to stash@{0}.
- `target.reinstateIndex`: uses --index.
- `timeoutMs`.
...
- Do not bypass `createBaseToolRegistry().lookupHandler("git.popStashChanges")` and `handler.invoke(...)` in integration tests.
