...
`Praxis_Agent_Architecture/src/agentCore/`
...
- 1 个共享测试助手：
  `Praxis_Agent_Architecture/test/agentCore/agentCoreContractTestHelper.ts`
- 1 个子工程配置：
  `Praxis_Agent_Architecture/package.json`
- 1 个子工程 TypeScript 配置：
  `Praxis_Agent_Architecture/tsconfig.json`

当前可运行：

```bash
cd /home/proview/Desktop/Praxis_series/Praxis_org/Praxis_Agent_Architecture
npm run typecheck
...
codex exec \
  ...
  --full-auto \
  -m gpt-5.4 \
  "按 Praxis_Agent_Architecture/Tasks/agentCore_parallel_implementation_plan.md 的 Slice A 实现 agentCore 第一轮最小主体。只改 Slice A 范围文件。完成后运行 cd Praxis_Agent_Architecture && npm run typecheck && npm run test:agentCore。"
```
...
`Praxis_Agent_Architecture/Tasks/run_agentcore_slices.mjs`
