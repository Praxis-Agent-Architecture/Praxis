...
> 对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_executionEngine/coreLogic/reuseInvoker.ts`

## 1. 文件位置

- 所属顶层模块：执行引擎（`agent_executionEngine`）。
- 所属路径：`agent_executionEngine/coreLogic`。
- 当前文件：`reuseInvoker.ts`。
- 角色概括：Agent 的执行身体，负责输入输出、PromptPack、主循环、状态机、基础工具原语和执行事件暴露。

## 2. 文件职责

承托 AgentCore 复用调用能力，让上层应用、CMP、MP、TAP 可以把 agentCore 打包复用。

这个文件的核心不是“占一个目录位置”，而是要在当前路径上形成一个可实现、可测试、可被 runtime 或相邻模块调用的窄能力点。它应该围绕“承托 AgentCore 复用调用能力，让上层应用、CMP、MP、TAP 可以把 agentCore 打包复用”建立清晰的输入、输出、错误和治理边界。

## 2.1 文件名语义拆解

- 原始文件名：`reuseInvoker.ts`。
- 命名片段：`reuse` / `Invoker`。
...
- 承托 AgentCore 复用调用能力，让上层应用、CMP、MP、TAP 可以把 agentCore 打包复用
- 更偏向复用已构建的 Agent 实例、能力集合或运行对象，而不是单纯缓存一次调用结果。
- 需要服务未来 OAO 场景：用户可以 new 出一个 agentCore 对象并用它承接实际 Agent。
- 需要让官方模块也按同一复用方式实践 agentCore，而不是各自绕开核心。
...
- 执行“承托 AgentCore 复用调用能力，让上层应用、CMP、MP、TAP 可以把 agentCore 打包复用”所需的任务输入、状态输入、事件输入和下一跳调用约束。
...
- 实现一个最小纯函数或薄类壳，能完成“承托 AgentCore 复用调用能力，让上层应用、CMP、MP、TAP 可以把 agentCore 打包复用”的可测路径。
...
- 验证该文件确实只完成“承托 AgentCore 复用调用能力，让上层应用、CMP、MP、TAP 可以把 agentCore 打包复用”，没有越界承担相邻模块职责。
