...
> 对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_executionEngine/basic_toolLayer/invocationStrategy.ts`

## 1. 文件位置

- 所属顶层模块：执行引擎（`agent_executionEngine`）。
- 所属路径：`agent_executionEngine/basic_toolLayer`。
- 当前文件：`invocationStrategy.ts`。
- 角色概括：Agent 的执行身体，负责输入输出、PromptPack、主循环、状态机、基础工具原语和执行事件暴露。

## 2. 文件职责

选择基础工具调用策略、执行模式和治理前置条件。

这个文件的核心不是“占一个目录位置”，而是要在当前路径上形成一个可实现、可测试、可被 runtime 或相邻模块调用的窄能力点。它应该围绕“选择基础工具调用策略、执行模式和治理前置条件”建立清晰的输入、输出、错误和治理边界。

## 2.1 文件名语义拆解

- 原始文件名：`invocationStrategy.ts`。
- 命名片段：`invocation` / `Strategy`。
- 工程含义：这是基础工具层的 `invocationStrategy` 位点，重点是支撑工具原语被调用、治理、存储或转交。
...
- 执行“选择基础工具调用策略、执行模式和治理前置条件”所需的工具参数、工作目录、审计上下文和 TAP 交接约束。

输入边界必须窄：只接收完成本文件职责所需的材料，不把相邻模块的大对象整包吞进来。

## 7. 输出边界

- “选择基础工具调用策略、执行模式和治理前置条件”后形成的工具层标准结果、状态、事件和审计材料。
...
- 实现一个最小纯函数或薄类壳，能完成“选择基础工具调用策略、执行模式和治理前置条件”的可测路径。
...
- 验证该文件确实只完成“选择基础工具调用策略、执行模式和治理前置条件”，没有越界承担相邻模块职责。
