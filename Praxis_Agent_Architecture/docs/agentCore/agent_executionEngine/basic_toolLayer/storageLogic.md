...
管理基础工具运行材料的存取、过期、复用和隔离逻辑。

这个文件的核心不是“占一个目录位置”，而是要在当前路径上形成一个可实现、可测试、可被 runtime 或相邻模块调用的窄能力点。它应该围绕“管理基础工具运行材料的存取、过期、复用和隔离逻辑”建立清晰的输入、输出、错误和治理边界。
...
- 执行“管理基础工具运行材料的存取、过期、复用和隔离逻辑”所需的工具参数、工作目录、审计上下文和 TAP 交接约束。
...
- 实现一个最小纯函数或薄类壳，能完成“管理基础工具运行材料的存取、过期、复用和隔离逻辑”的可测路径。
...
- 验证该文件确实只完成“管理基础工具运行材料的存取、过期、复用和隔离逻辑”，没有越界承担相邻模块职责。
