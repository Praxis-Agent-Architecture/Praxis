...
对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/screenRecording/computeruse.fullscreenScreenRecording.ts`

## 1. 文件位置

- 所属模块：`agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/screenRecording`。
- 当前 entry：`computeruse.fullscreenScreenRecording.ts`。
- storage 实现：`src/storagePool/baseToolStorage/computeruseBase/screenRecording/computeruse.fullscreenScreenRecording/`。

## 2. 文件职责

公开全屏录制基础工具的 canonical storage 实现、handler、definition 和类型。
...
- `fullscreenScreenRecording`：请求 runtime 开始一个全屏录制 session。
...
- runtime 负责屏幕访问、录屏流、媒体编码、session handle、artifact 存储、隐私边界和清理。
...
- 导出 `fullscreenScreenRecordingHandler` 供 registry 挂载。
- 导出 `fullscreenScreenRecordingBaseToolDefinition` 供工具发现和描述使用。
- 导出 `planFullscreenScreenRecording` / `executeFullscreenScreenRecording` 供兼容测试和局部调用。
- 导出 public-safe 类型，支撑 runtime inspection 和 TAP 高级工具组合。

## 6. 输入边界

- `purpose`：开始录屏的目的。
- `target.displayId` 或 `displayId`：目标显示器。
- `target.maxDurationMs` / `includeCursor` / `includeAudio` / `outputFormat`：录制 session 参数。
...
- 非法 displayId、duration、boolean、format 或 destinationHint 返回对应 invalid 错误。
...
- `BaseToolExecutorPort.computeruse.startRecording`：唯一真实全屏录制 session 启动入口。
- `runtime.governancePlane.toolInvocationGrant`：真实执行 guard 来源。
- `runtime.recordingSession.screen`：录屏流、session handle、artifact 和清理所有权。
- TAP 高级工具系统：负责编排、审批 UX 和 fallback 策略。

## 10. 被谁调用

- `createBaseToolRegistry().lookupHandler("computeruse.fullscreenScreenRecording")`。
...
- storage `core.ts` 负责 JSON 校验、target normalization、duration/format/destination validation、dry-run、guard、provider missing/failure 和输出归一化。
...
`computeruse.fullscreenScreenRecording` 是底层能力承载面。TAP/agent 可以把它和 `omniBase`、`mcpBase`、`shellBase` 组合，但这个 baseTool 自己不做策略编排。

完整 ToolSkill 调用手册放在 storage 文档：

`src/storagePool/baseToolStorage/computeruseBase/screenRecording/computeruse.fullscreenScreenRecording/computeruse.fullscreenScreenRecording.md`
