...
对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/keyboardEmulation/computeruse.keyboardInputEmulation.ts`

## 1. 文件位置

- 所属模块：`agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/keyboardEmulation`。
- 当前 entry：`computeruse.keyboardInputEmulation.ts`。
- storage 实现：`src/storagePool/baseToolStorage/computeruseBase/keyboardEmulation/computeruse.keyboardInputEmulation/`。

## 2. 文件职责

公开键盘文本输入基础工具的 canonical storage 实现、handler、definition 和类型。
...
- `keyboardInputEmulation`：请求 runtime 向当前受治理的键盘焦点目标输入文本。
- 这是基础工具原语，不是 TAP 的高级工具，也不决定是否先用 shell、MCP、browser-use、clipboard 或 omni。
...
- 导出 `keyboardInputEmulationHandler` 供 registry 挂载。
- 导出 `keyboardInputEmulationBaseToolDefinition` 供工具发现和描述使用。
- 导出 `planKeyboardInputEmulation` / `executeKeyboardInputEmulation` 供兼容测试和局部调用。
- 导出 public-safe 类型，支撑 runtime inspection 和 TAP 高级工具组合。

## 6. 输入边界

- `purpose`：输入文本的目的。
- `target.text` 或 `text`：需要输入的文本。
- `target.inputMode` 或 `inputMode`：`text` 或 `paste`，但 runtime port 仍接收 primitive `type` action。
...
- dry-run 返回 metadata-only keyboard action envelope，并包含文本长度、输入模式和 runtime entry。
...
- 缺 runtimeId、purpose 或 text 返回 `MISSING_RUNTIME_ID` / `MISSING_PURPOSE` / `MISSING_TEXT`。
- 非法 inputMode、targetHint、text limit 或超限文本返回对应 invalid/resource 错误。
...
- `BaseToolExecutorPort.computeruse.keyboardAction`：唯一真实键盘文本输入入口。
...
- `createBaseToolRegistry().lookupHandler("computeruse.keyboardInputEmulation")`。
- runtime execEngine 的 baseTool invocation bridge。
- TAP 高级工具系统和 agent 编排层。

## 11. 不应该做什么

- 不要在 entry 或 storage 中隐藏调用 shell、portal、本地键盘命令、clipboard helper 或浏览器自动化。
- 不要在 baseTool 中决定是否优先用 MCP、shell、browser-use、clipboard 或 omni。
- 不要在 baseTool 中解析真实焦点、发 OS 键盘事件或持有长生命周期输入会话。

## 12. 最小实现建议

- entry 必须保持显式导出，不使用 bare `export *`。
- storage `core.ts` 负责 JSON 校验、text/inputMode/targetHint normalization、dry-run、guard、provider missing/failure 和输出归一化。
...
`computeruse.keyboardInputEmulation` 是底层能力承载面。TAP/agent 可以把它和截图、mouse、MCP、shell、omni 或 browser runtime 组合，但这个 baseTool 自己不做策略编排。

完整 ToolSkill 调用手册放在 storage 文档：

`src/storagePool/baseToolStorage/computeruseBase/keyboardEmulation/computeruse.keyboardInputEmulation/computeruse.keyboardInputEmulation.md`
