...
对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/cameraAccess/computeruse.cameraSelect.ts`

## 1. 文件位置

- 所属模块：`agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/cameraAccess`。
- 当前 entry：`computeruse.cameraSelect.ts`。
- storage 实现：`src/storagePool/baseToolStorage/computeruseBase/cameraAccess/computeruse.cameraSelect/`。

## 2. 文件职责

公开摄像头设备选择基础工具的 canonical storage 实现、handler、definition 和类型。
...
- `cameraSelect`：请求 runtime 选择一个摄像头设备作为后续 camera 原语的 active device。
...
- runtime 负责摄像头枚举、OS/device policy、active device 状态、privacy cleanup 和真实设备选择。
...
- 导出 `cameraSelectHandler` 供 registry 挂载。
- 导出 `cameraSelectBaseToolDefinition` 供工具发现和描述使用。
- 导出 `planCameraSelect` / `executeCameraSelect` 供兼容测试和局部调用。
- 导出 public-safe 类型，支撑 runtime inspection 和 TAP 高级工具组合。

## 6. 输入边界

- `target.deviceId` 或 `deviceId`：要选择的 runtime 摄像头设备 id。
- `target.availableDevices` 或 `availableDevices`：可选 runtime 候选设备列表；提供时用于校验 deviceId。
- `target.purpose` 或 `purpose`：选择设备的审计理由。
- `context.dryRun` / `context.guard` / scope 字段：执行边界和治理材料。
- `BaseToolInvokeRequest.runtimeId/sessionId/toolCallId`：handler 路径注入的运行态元数据。

## 7. 输出边界

- dry-run 返回 metadata-only selection envelope，并包含 normalized target。
- 真实执行成功返回 `requested`、`selected`、selected `deviceId`、runtime entry 和 audit metadata。
- 输出不包含摄像头画面、不包含 provider 私有错误、不包含 TAP 策略判断。

## 8. 错误边界

- malformed JSON 返回 `INVALID_REQUEST` / `INVALID_CONTEXT` / `INVALID_TARGET`。
- 缺 runtimeId 或 camera device 返回对应 missing 错误。
- 非法 deviceId、非法 availableDevices、候选列表不包含目标设备返回对应 invalid/resource 错误。
...
- `BaseToolExecutorPort.computeruse.selectDevice`：唯一真实摄像头设备选择入口。
- `runtime.governancePlane.toolInvocationGrant`：真实执行 guard 来源。
- `runtime.devicePolicy.camera`：设备枚举、OS/device policy、active device 状态和 cleanup 所有权。
- TAP 高级工具系统：负责编排、审批 UX 和 fallback 策略。

## 10. 被谁调用

- `createBaseToolRegistry().lookupHandler("computeruse.cameraSelect")`。
...
- storage `core.ts` 负责 JSON 校验、target normalization、availableDevices validation、dry-run、guard、provider missing/failure 和输出归一化。
...
`computeruse.cameraSelect` 是 cameraAccess 的设备选择原语。TAP/agent 可以把它和 camera permission request、capture/recording、`omniBase`、`mcpBase`、`shellBase` 组合，但这个 baseTool 自己不做策略编排。

完整 ToolSkill 调用手册放在 storage 文档：

`src/storagePool/baseToolStorage/computeruseBase/cameraAccess/computeruse.cameraSelect/computeruse.cameraSelect.md`
...
- `Runtime Behavior` 必须写清真实副作用只走 `BaseToolExecutorPort.computeruse.selectDevice`，缺 runtime provider 返回 `PROVIDER_UNAVAILABLE`。
