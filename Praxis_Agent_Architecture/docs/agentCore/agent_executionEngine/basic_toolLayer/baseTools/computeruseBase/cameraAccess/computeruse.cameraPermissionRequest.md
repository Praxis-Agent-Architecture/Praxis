...
对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/cameraAccess/computeruse.cameraPermissionRequest.ts`

## 1. 文件位置

- 所属模块：`agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/cameraAccess`。
- 当前 entry：`computeruse.cameraPermissionRequest.ts`。
- storage 实现：`src/storagePool/baseToolStorage/computeruseBase/cameraAccess/computeruse.cameraPermissionRequest/`。

## 2. 文件职责

公开摄像头权限申请基础工具的 canonical storage 实现、handler、definition 和类型。
...
- `cameraPermissionRequest`：请求 runtime 为摄像头访问申请 permission lease。
...
- runtime 负责 OS 摄像头权限提示、device lease、app/device allowlist、隐私边界和清理。
...
- 导出 `cameraPermissionRequestHandler` 供 registry 挂载。
- 导出 `cameraPermissionRequestBaseToolDefinition` 供工具发现和描述使用。
- 导出 `planCameraPermissionRequest` / `executeCameraPermissionRequest` 供兼容测试和局部调用。
- 导出 public-safe 类型，支撑 runtime inspection 和 TAP 高级工具组合。

## 6. 输入边界

- `target.targetApplication` 或 `targetApplication`：需要摄像头权限的应用或 workflow surface。
- `target.purpose` 或 `purpose`：申请摄像头权限的明确目的。
- `target.deviceId` 或 `deviceId`：可选 runtime 摄像头设备 id。
- `target.mode` / `target.requestedDurationMs`：permission lease 的使用模式和时长边界。
- `context.dryRun` / `context.guard` / scope 字段：执行边界和治理材料。
- `BaseToolInvokeRequest.runtimeId/sessionId/toolCallId`：handler 路径注入的运行态元数据。

## 7. 输出边界

- dry-run 返回 metadata-only permission envelope，并包含 normalized target。
- 真实执行成功返回 `requested`、`granted`、可选 `leaseId`、runtime entry 和 audit metadata。
- 输出不包含摄像头画面、不包含 provider 私有错误、不包含 TAP 策略判断。

## 8. 错误边界

- malformed JSON 返回 `INVALID_REQUEST` / `INVALID_CONTEXT` / `INVALID_TARGET`。
- 缺 runtimeId、targetApplication 或 purpose 返回对应 missing 错误。
- 非法 deviceId、mode 或 duration 返回对应 invalid/resource 错误。
...
- `BaseToolExecutorPort.computeruse.requestPermission`：唯一真实摄像头 permission request 入口。
- `runtime.governancePlane.toolInvocationGrant`：真实执行 guard 来源。
- `runtime.devicePolicy.camera`：OS 权限提示、device lease、privacy boundary 和 cleanup 所有权。
- TAP 高级工具系统：负责编排、审批 UX 和 fallback 策略。

## 10. 被谁调用

- `createBaseToolRegistry().lookupHandler("computeruse.cameraPermissionRequest")`。
- runtime execEngine 的 baseTool invocation bridge。
- TAP 高级工具系统和 agent 编排层。

## 11. 不应该做什么

- 不要在 entry 或 storage 中隐藏调用 shell、portal、本地摄像头命令、浏览器自动化或 provider SDK。
- 不要在 baseTool 中决定是否优先用 MCP、shell、browser-use 或 omni。
- 不要捕获、读取或返回摄像头画面；后续 photo/video artifact 由其他 camera 工具和 runtime media adapter 承载。

## 12. 最小实现建议

- entry 必须保持显式导出，不使用 bare `export *`。
- storage `core.ts` 负责 JSON 校验、target normalization、duration validation、dry-run、guard、provider missing/failure 和输出归一化。
...
`computeruse.cameraPermissionRequest` 是 cameraAccess 的底层准入原语。TAP/agent 可以把它和 camera capture/recording、`omniBase`、`mcpBase`、`shellBase` 组合，但这个 baseTool 自己不做策略编排。

完整 ToolSkill 调用手册放在 storage 文档：

`src/storagePool/baseToolStorage/computeruseBase/cameraAccess/computeruse.cameraPermissionRequest/computeruse.cameraPermissionRequest.md`
...
- `Runtime Behavior` 必须写清真实副作用只走 `BaseToolExecutorPort.computeruse.requestPermission`，缺 runtime provider 返回 `PROVIDER_UNAVAILABLE`。
