...
对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/cameraAccess/computeruse.cameraContentStorage.ts`

## 1. 文件位置

- 所属模块：`agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/cameraAccess`。
- 当前 entry：`computeruse.cameraContentStorage.ts`。
- storage 实现：`src/storagePool/baseToolStorage/computeruseBase/cameraAccess/computeruse.cameraContentStorage/`。

## 2. 文件职责

公开摄像头内容存储基础工具的 canonical storage 实现、handler、definition 和类型。
...
- `cameraContentStorage`：请求 runtime 存储已有 camera content artifact reference。
...
- runtime 负责摄像头内容字节、artifact store、retention policy、隐私边界和 cleanup。
...
- 导出 `cameraContentStorageHandler` 供 registry 挂载。
- 导出 `cameraContentStorageBaseToolDefinition` 供工具发现和描述使用。
- 导出 `planCameraContentStorage` / `executeCameraContentStorage` 供兼容测试和局部调用。
- 导出 public-safe 类型，支撑 runtime inspection 和 TAP 高级工具组合。

## 6. 输入边界

- `target.contentRef` / `cameraContentRef` / `artifactRef` 或顶层同名字段：runtime 摄像头内容 artifact reference。
- `target.storageTarget` 或 `storageTarget`：存储目标，只允许 `artifact://`、`session://`、`runtime://`、`memory://`。
- `purpose`：存储目的。
- `target.contentKind` / `retentionPolicy`：可选 camera artifact 类型和保留策略。
- `context.dryRun` / `context.guard` / scope 字段：执行边界和治理材料。
- `BaseToolInvokeRequest.runtimeId/sessionId/toolCallId`：handler 路径注入的运行态元数据。

## 7. 输出边界

- dry-run 返回 metadata-only storage envelope，并包含 normalized target。
- 真实执行成功返回 `stored`、`storedArtifactId`、runtime entry 和 audit metadata。
- 输出不包含摄像头字节、不包含 provider 私有错误、不包含 TAP 策略判断。

## 8. 错误边界

- malformed JSON 返回 `INVALID_REQUEST` / `INVALID_CONTEXT` / `INVALID_TARGET`。
- 缺 runtimeId、contentRef、storageTarget 或 purpose 返回对应 missing 错误。
- 非法 contentRef、contentKind、storageTarget 或 retentionPolicy 返回对应 invalid 错误。
...
- `BaseToolExecutorPort.artifact.store`：唯一真实摄像头内容存储入口。
- `runtime.governancePlane.toolInvocationGrant`：真实执行 guard 来源。
- `runtime.artifactStore.cameraContent`：camera bytes、photo/video material、artifact retention、privacy boundary 和 cleanup 所有权。
- TAP 高级工具系统：负责编排、审批 UX、后续 artifact 消费和 fallback 策略。

## 10. 被谁调用

- `createBaseToolRegistry().lookupHandler("computeruse.cameraContentStorage")`。
...
- 不要捕获照片、停止录制、读取最终视频内容、做人脸识别或图像理解；这些是其他 camera/vision/omni 能力。

## 12. 最小实现建议

- entry 必须保持显式导出，不使用 bare `export *`。
- storage `core.ts` 负责 JSON 校验、target normalization、content/storage/retention validation、dry-run、guard、provider missing/failure 和输出归一化。
...
`computeruse.cameraContentStorage` 是 cameraAccess 的 artifact storage 原语。TAP/agent 可以把它和 camera capture photo、camera stop recording、`omniBase`、`mcpBase`、`shellBase` 组合，但这个 baseTool 自己不做策略编排。

完整 ToolSkill 调用手册放在 storage 文档：

`src/storagePool/baseToolStorage/computeruseBase/cameraAccess/computeruse.cameraContentStorage/computeruse.cameraContentStorage.md`
