...
对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/cameraAccess/computeruse.cameraStopRecording.ts`

## 1. 文件位置

- 所属模块：`agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/cameraAccess`。
- 当前 entry：`computeruse.cameraStopRecording.ts`。
- storage 实现：`src/storagePool/baseToolStorage/computeruseBase/cameraAccess/computeruse.cameraStopRecording/`。

## 2. 文件职责

公开摄像头停止录制基础工具的 canonical storage 实现、handler、definition 和类型。
...
- `cameraStopRecording`：请求 runtime 停止摄像头录制并返回视频 artifact reference。
...
- runtime 负责摄像头 stream、录制 session handle、codec、media artifact 生命周期、retention policy 和 cleanup。
...
- 导出 `cameraStopRecordingHandler` 供 registry 挂载。
- 导出 `cameraStopRecordingBaseToolDefinition` 供工具发现和描述使用。
- 导出 `planCameraStopRecording` / `executeCameraStopRecording` 供兼容测试和局部调用。
- 导出 public-safe 类型，支撑 runtime inspection 和 TAP 高级工具组合。

## 6. 输入边界

- `target.recordingId` / `recordingRef` 或顶层同名字段：runtime 摄像头录制 session handle。
- `target.purpose` 或 `purpose`：停止录制的目的。
- `target.storageTarget` / `retentionPolicy` / `destinationHint`：可选 runtime artifact 存储元数据。
...
- 真实执行成功返回 `stopRequested`、`stopped`、`artifactId`、`mimeType`、runtime entry 和 audit metadata。
- 输出不包含视频二进制、不包含 provider 私有错误、不包含 TAP 策略判断。

## 8. 错误边界

- malformed JSON 返回 `INVALID_REQUEST` / `INVALID_CONTEXT` / `INVALID_TARGET`。
- 缺 runtimeId、recordingId 或 purpose 返回对应 missing 错误。
- 非法 recordingId、purpose、storageTarget、retentionPolicy 或 destination hint 返回对应 invalid 错误。
...
- `BaseToolExecutorPort.computeruse.stopRecording`：唯一真实摄像头停止录制入口。
- `runtime.governancePlane.toolInvocationGrant`：真实执行 guard 来源。
- `runtime.mediaSession.cameraRecording`：camera stream、recording session handle、codec、artifact lifecycle、retention policy 和 cleanup 所有权。
- TAP 高级工具系统：负责编排、审批 UX、后续 artifact 消费和 fallback 策略。

## 10. 被谁调用

- `createBaseToolRegistry().lookupHandler("computeruse.cameraStopRecording")`。
...
- 不要开始录制、读取最终视频内容、做人脸识别或图像理解；这些是其他 camera/vision/omni 能力。

## 12. 最小实现建议

- entry 必须保持显式导出，不使用 bare `export *`。
- storage `core.ts` 负责 JSON 校验、target normalization、recording/storage/retention validation、dry-run、guard、provider missing/failure 和输出归一化。
...
`computeruse.cameraStopRecording` 是 cameraAccess 的 recording session finalization 原语。TAP/agent 可以把它和 camera permission request、camera select、camera start recording、`omniBase`、`mcpBase`、`shellBase` 组合，但这个 baseTool 自己不做策略编排。

完整 ToolSkill 调用手册放在 storage 文档：

`src/storagePool/baseToolStorage/computeruseBase/cameraAccess/computeruse.cameraStopRecording/computeruse.cameraStopRecording.md`
...
- `Runtime Behavior` 必须写清真实副作用只走 `BaseToolExecutorPort.computeruse.stopRecording`，缺 runtime provider 返回 `PROVIDER_UNAVAILABLE`。
