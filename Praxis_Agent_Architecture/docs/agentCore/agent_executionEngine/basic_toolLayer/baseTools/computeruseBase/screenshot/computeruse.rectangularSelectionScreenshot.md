...
对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/screenshot/computeruse.rectangularSelectionScreenshot.ts`

## 1. 文件位置

- 所属模块：`agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/screenshot`。
- 当前 entry：`computeruse.rectangularSelectionScreenshot.ts`。
- storage 实现：`src/storagePool/baseToolStorage/computeruseBase/screenshot/computeruse.rectangularSelectionScreenshot/`。

## 2. 文件职责

公开矩形区域截图基础工具的 canonical storage 实现、handler、definition 和类型。
...
- `rectangularSelectionScreenshot`：请求 runtime 捕获指定矩形区域截图 artifact。
...
- runtime 负责真实区域捕获、屏幕访问、权限、截图字节和 artifact 存储。
...
- 导出 `rectangularSelectionScreenshotHandler` 供 registry 挂载。
- 导出 `rectangularSelectionScreenshotBaseToolDefinition` 供工具发现和描述使用。
- 导出 `planRectangularSelectionScreenshot` / `executeRectangularSelectionScreenshot` 供兼容测试和局部调用。
- 导出 public-safe 类型，支撑 runtime inspection 和 TAP 高级工具组合。

## 6. 输入边界

- `purpose`：截图目的。
- `target.rect` 或 `rect`：矩形区域坐标。
...
- 缺 runtimeId、purpose 或 rect 返回 `MISSING_RUNTIME_ID` / `MISSING_PURPOSE` / `MISSING_RECT`。
- 非法坐标或区域过大返回 `INVALID_RECT` / `RECT_TOO_LARGE`。
...
- `BaseToolExecutorPort.computeruse.captureScreenshot`：唯一真实区域截图入口。
...
- `createBaseToolRegistry().lookupHandler("computeruse.rectangularSelectionScreenshot")`。
...
`computeruse.rectangularSelectionScreenshot` 是底层能力承载面。TAP/agent 可以把它和 `omniBase`、`mcpBase`、`shellBase` 组合，但这个 baseTool 自己不做策略编排。

完整 ToolSkill 调用手册放在 storage 文档：

`src/storagePool/baseToolStorage/computeruseBase/screenshot/computeruse.rectangularSelectionScreenshot/computeruse.rectangularSelectionScreenshot.md`
