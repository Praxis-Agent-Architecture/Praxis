...
对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/screenshot/computeruse.screenshotStorage.ts`

## 1. 文件位置

- 所属模块：`agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/screenshot`。
- 当前 entry：`computeruse.screenshotStorage.ts`。
- storage 实现：`src/storagePool/baseToolStorage/computeruseBase/screenshot/computeruse.screenshotStorage/`。

## 2. 文件职责

公开截图 artifact 存储基础工具的 canonical storage 实现、handler、definition 和类型。
...
- `screenshotStorage`：请求 runtime 存储、保留或提升一个已有截图 artifact。
...
- runtime 负责截图字节、artifact handle、retention policy、隐私边界、持久化和清理。
...
- 导出 `screenshotStorageHandler` 供 registry 挂载。
- 导出 `screenshotStorageBaseToolDefinition` 供工具发现和描述使用。
- 导出 `planScreenshotStorage` / `executeScreenshotStorage` 供兼容测试和局部调用。
- 导出 public-safe 类型，支撑 runtime inspection 和 TAP 高级工具组合。

## 6. 输入边界

- `purpose`：存储截图 artifact 的目的。
- `target.screenshotRef` 或 `screenshotRef`：已有截图 artifact 引用。
- `target.storageTarget` 或 `storageTarget`：目标 artifact scope，必须是 `artifact://`、`session://`、`runtime://` 或 `memory://`。
- `target.retentionPolicy` 或 `retentionPolicy`：retention 策略。
- `context.dryRun` / `context.guard` / scope 字段：执行边界和治理材料。
- `BaseToolInvokeRequest.runtimeId/sessionId/toolCallId`：handler 路径注入的运行态元数据。

## 7. 输出边界

- dry-run 返回 metadata-only storage envelope，并包含 normalized screenshot/storage target。
- 真实执行成功返回 `storedArtifactId`、`storageUri`、retention policy、runtime entry 和 audit metadata。
- 输出不包含截图字节，不包含 provider 私有错误，不包含 TAP 策略判断。

## 8. 错误边界

- malformed JSON 返回 `INVALID_REQUEST` / `INVALID_CONTEXT` / `INVALID_TARGET`。
- 缺 runtimeId、purpose、screenshotRef 或 storageTarget 返回对应 missing 错误。
- 非法 screenshotRef、storageTarget 或 retentionPolicy 返回对应 invalid 错误。
...
- `BaseToolExecutorPort.artifact.store`：唯一真实 artifact 存储入口。
- `runtime.governancePlane.toolInvocationGrant`：真实执行 guard 来源。
- `runtime.artifactStore.screenCapture`：截图字节、artifact retention 和清理所有权。
- TAP 高级工具系统：负责编排、审批 UX 和 fallback 策略。

## 10. 被谁调用

- `createBaseToolRegistry().lookupHandler("computeruse.screenshotStorage")`。
- runtime execEngine 的 baseTool invocation bridge。
- TAP 高级工具系统和 agent 编排层。

## 11. 不应该做什么

- 不要在 entry 或 storage 中隐藏调用 shell、filesystem、本地截图命令、portal 或浏览器自动化。
- 不要在 baseTool 中决定是否优先用 MCP、shell、browser-use 或 omni。
- 不要读取、写入或返回截图字节；runtime 持有 artifact，omni/modelAdapter 可在上层消费。

## 12. 最小实现建议

- entry 必须保持显式导出，不使用 bare `export *`。
- storage `core.ts` 负责 JSON 校验、artifact reference validation、storage target validation、dry-run、guard、provider missing/failure 和输出归一化。
...
`computeruse.screenshotStorage` 是底层能力承载面。TAP/agent 可以把它和 `omniBase`、`mcpBase`、`shellBase` 组合，但这个 baseTool 自己不做策略编排。

完整 ToolSkill 调用手册放在 storage 文档：

`src/storagePool/baseToolStorage/computeruseBase/screenshot/computeruse.screenshotStorage/computeruse.screenshotStorage.md`
...
- `Runtime Behavior` 必须写清真实副作用只走 `BaseToolExecutorPort.artifact.store`，缺 runtime provider 返回 `PROVIDER_UNAVAILABLE`。
- `Avoid` 必须写清不隐藏 shell、filesystem、portal、browser-use、MCP、media/device 依赖，也不自动调用 `omniBase`。
