...
对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/screenshot/computeruse.fullscreenScreenshot.ts`

## 1. 文件位置

- 所属模块：`agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/screenshot`。
- 当前 entry：`computeruse.fullscreenScreenshot.ts`。
- storage 实现：`src/storagePool/baseToolStorage/computeruseBase/screenshot/computeruse.fullscreenScreenshot/`。

## 2. 文件职责

公开全屏截图基础工具的 canonical storage 实现、handler、definition 和类型。
...
- `fullscreenScreenshot`：请求 runtime 捕获全屏截图 artifact。
...
- runtime 负责真实屏幕访问、权限、截图字节和 artifact 存储。
...
- 导出 `fullscreenScreenshotHandler` 供 registry 挂载。
- 导出 `fullscreenScreenshotBaseToolDefinition` 供工具发现和描述使用。
- 导出 `planFullscreenScreenshot` / `executeFullscreenScreenshot` 供兼容测试和局部调用。
...
- `BaseToolExecutorPort.computeruse.captureScreenshot`：唯一真实截图入口。
...
- `createBaseToolRegistry().lookupHandler("computeruse.fullscreenScreenshot")`。
...
`computeruse.fullscreenScreenshot` 是底层能力承载面。TAP/agent 可以把它和 `omniBase`、`mcpBase`、`shellBase` 组合，但这个 baseTool 自己不做策略编排。
