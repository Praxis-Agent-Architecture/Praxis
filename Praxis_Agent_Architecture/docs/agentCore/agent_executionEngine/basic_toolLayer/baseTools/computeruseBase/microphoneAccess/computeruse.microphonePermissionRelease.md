...
对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/microphoneAccess/computeruse.microphonePermissionRelease.ts`

## 1. 文件位置

- 所属模块：`agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/microphoneAccess`。
- 当前 entry：`computeruse.microphonePermissionRelease.ts`。
- storage 实现：`src/storagePool/baseToolStorage/computeruseBase/microphoneAccess/computeruse.microphonePermissionRelease/`。

## 2. 文件职责

公开麦克风权限释放基础工具的 canonical storage 实现、handler、definition 和类型。
...
- `microphonePermissionRelease`：请求 runtime 释放一个已有麦克风 permission lease。
...
- 导出 `microphonePermissionReleaseHandler` 供 registry 挂载。
- 导出 `microphonePermissionReleaseBaseToolDefinition` 供工具发现和描述使用。
- 导出 `planMicrophonePermissionRelease` / `executeMicrophonePermissionRelease` 供兼容测试和局部调用。
- 导出 public-safe 类型，支撑 runtime inspection 和 TAP 高级工具组合。

## 6. 输入边界

- `target.permissionLeaseId` 或 `permissionLeaseId`：需要释放的麦克风权限 lease。
- `target.targetApplication` 或 `targetApplication`：释放权限的应用或工作流面。
- `target.deviceId` / `target.releaseReason`：可选设备和释放原因。
- `context.dryRun` / `context.guard` / scope 字段：执行边界和治理材料。
- `BaseToolInvokeRequest.runtimeId/sessionId/toolCallId`：handler 路径注入的运行态元数据。

## 7. 输出边界

- dry-run 返回 metadata-only release envelope，不触发 provider。
- 真实执行成功返回 `requested`、`released`、`leaseId`、runtime entry 和 audit metadata。
- 输出不包含音频数据，不包含 provider 私有错误，不包含 TAP 策略判断。

## 8. 错误边界

- malformed JSON 返回 `INVALID_REQUEST` / `INVALID_CONTEXT` / `INVALID_TARGET`。
- 缺 runtimeId、permissionLeaseId 或 targetApplication 返回对应 missing 错误。
- 非法 lease、deviceId 或 releaseReason 返回对应 invalid 错误。
...
- `BaseToolExecutorPort.computeruse.releasePermission`：唯一真实麦克风权限释放入口。
...
- `createBaseToolRegistry().lookupHandler("computeruse.microphonePermissionRelease")`。
...
- 不要录音、读取音频流或返回音频 artifact；权限释放和录音是不同原语。

## 12. 最小实现建议

- entry 必须保持显式导出，不使用 bare `export *`。
- storage `core.ts` 负责 JSON 校验、target normalization、lease validation、dry-run、guard、provider missing/failure 和输出归一化。
...
`computeruse.microphonePermissionRelease` 是底层能力承载面。TAP/agent 可以把它和 `omniBase`、`mcpBase`、`shellBase` 组合，但这个 baseTool 自己不做策略编排。

完整 ToolSkill 调用手册放在 storage 文档：

`src/storagePool/baseToolStorage/computeruseBase/microphoneAccess/computeruse.microphonePermissionRelease/computeruse.microphonePermissionRelease.md`
...
- `Runtime Behavior` 必须写清真实副作用只走 `BaseToolExecutorPort.computeruse.releasePermission`，缺 runtime provider 返回 `PROVIDER_UNAVAILABLE`。
