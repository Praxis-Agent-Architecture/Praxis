...
对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/microphoneAccess/computeruse.microphonePermissionRequest.ts`

## 1. 文件位置

- 所属模块：`agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/microphoneAccess`。
- 当前 entry：`computeruse.microphonePermissionRequest.ts`。
- storage 实现：`src/storagePool/baseToolStorage/computeruseBase/microphoneAccess/computeruse.microphonePermissionRequest/`。

## 2. 文件职责

公开麦克风权限申请基础工具的 canonical storage 实现、handler、definition 和类型。
...
- `microphonePermissionRequest`：请求 runtime 为麦克风访问创建 permission grant 或 lease。
...
- runtime 负责 OS 权限提示、麦克风设备访问、device lease、隐私边界、授权撤销和清理。
...
- 导出 `microphonePermissionRequestHandler` 供 registry 挂载。
- 导出 `microphonePermissionRequestBaseToolDefinition` 供工具发现和描述使用。
- 导出 `planMicrophonePermissionRequest` / `executeMicrophonePermissionRequest` 供兼容测试和局部调用。
- 导出 public-safe 类型，支撑 runtime inspection 和 TAP 高级工具组合。

## 6. 输入边界

- `target.targetApplication` 或 `targetApplication`：需要麦克风访问的应用或工作流面。
- `target.purpose` 或 `purpose`：权限申请目的。
- `target.deviceId` / `target.mode` / `target.requestedDurationMs`：可选设备、模式和时长约束。
- `context.dryRun` / `context.guard` / scope 字段：执行边界和治理材料。
- `BaseToolInvokeRequest.runtimeId/sessionId/toolCallId`：handler 路径注入的运行态元数据。

## 7. 输出边界

- dry-run 返回 metadata-only permission envelope，不触发 provider。
- 真实执行成功返回 `requested`、`granted`、可选 `leaseId`、runtime entry 和 audit metadata。
- 输出不包含音频数据，不包含 provider 私有错误，不包含 TAP 策略判断。

## 8. 错误边界

- malformed JSON 返回 `INVALID_REQUEST` / `INVALID_CONTEXT` / `INVALID_TARGET`。
- 缺 runtimeId、targetApplication 或 purpose 返回对应 missing 错误。
- 非法 deviceId、mode、duration 或 duration 越界返回对应 invalid/resource 错误。
...
- `BaseToolExecutorPort.computeruse.requestPermission`：唯一真实麦克风权限申请入口。
- `runtime.governancePlane.toolInvocationGrant`：真实执行 guard 来源。
- `runtime.devicePolicy.microphone`：OS prompt、device lease、隐私边界和撤销所有权。
- TAP 高级工具系统：负责编排、审批 UX 和 fallback 策略。

## 10. 被谁调用

- `createBaseToolRegistry().lookupHandler("computeruse.microphonePermissionRequest")`。
- runtime execEngine 的 baseTool invocation bridge。
- TAP 高级工具系统和 agent 编排层。

## 11. 不应该做什么

- 不要在 entry 或 storage 中隐藏调用 shell、portal、ffmpeg、PipeWire、本地音频命令或浏览器自动化。
- 不要在 baseTool 中决定是否优先用 MCP、shell、browser-use 或 omni。
- 不要录音、读取音频流或返回音频 artifact；权限申请和录音是不同原语。
...
`computeruse.microphonePermissionRequest` 是底层能力承载面。TAP/agent 可以把它和 `omniBase`、`mcpBase`、`shellBase` 组合，但这个 baseTool 自己不做策略编排。

完整 ToolSkill 调用手册放在 storage 文档：

`src/storagePool/baseToolStorage/computeruseBase/microphoneAccess/computeruse.microphonePermissionRequest/computeruse.microphonePermissionRequest.md`
...
- `Avoid` 必须写清不隐藏 shell、portal、browser-use、MCP、ffmpeg、PipeWire、media/device 依赖，也不自动调用 `omniBase`。
