...
对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/mouseEmulation/computeruse.checkboxConfirm.ts`

## 1. 文件位置

- 所属模块：`agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/mouseEmulation`。
- 当前 entry：`computeruse.checkboxConfirm.ts`。
- storage 实现：`src/storagePool/baseToolStorage/computeruseBase/mouseEmulation/computeruse.checkboxConfirm/`。

## 2. 文件职责

公开复选框确认基础工具的 canonical storage 实现、handler、definition 和类型。
...
- `checkboxConfirm`：请求 runtime 将复选框确认到目标状态。
...
- runtime 负责真实 OS pointer events、目标解析、窗口焦点、权限、session handle 和清理。
...
- 导出 `checkboxConfirmHandler` 供 registry 挂载。
- 导出 `checkboxConfirmBaseToolDefinition` 供工具发现和描述使用。
- 导出 `planCheckboxConfirm` / `executeCheckboxConfirm` 供兼容测试和局部调用。
- 导出 public-safe 类型，支撑 runtime inspection 和 TAP 高级工具组合。

## 6. 输入边界

- `purpose`：确认复选框状态的目的。
- `target.label` / `target.selectorHint` / `target.point`：至少一种 target locator。
- `target.expectedState` / `target.currentState`：目标状态和可选当前状态。
- `target.coordinateSpace`：坐标空间，支持 `screen` / `window` / `normalized`。
- `target.clickMode`：支持 `single-click` / `double-click`。
- `context.dryRun` / `context.guard` / scope 字段：执行边界和治理材料。
- `BaseToolInvokeRequest.runtimeId/sessionId/toolCallId`：handler 路径注入的运行态元数据。

## 7. 输出边界

- dry-run 返回 metadata-only pointer confirm envelope，并包含 normalized target。
- 真实执行成功返回 `actionId`、可选 `finalState`、runtime entry 和 audit metadata。
...
- 缺 runtimeId、purpose 或 target locator 返回 `MISSING_RUNTIME_ID` / `MISSING_PURPOSE` / `MISSING_TARGET`。
- 非法 point、state、clickMode、coordinateSpace、displayId 或 windowId 返回对应 invalid 错误。
...
- `BaseToolExecutorPort.computeruse.pointerAction`：唯一真实复选框确认入口。
- `runtime.governancePlane.toolInvocationGrant`：真实执行 guard 来源。
- `runtime.inputController.pointer`：OS pointer events、目标解析、焦点、坐标转换和清理所有权。
- TAP 高级工具系统：负责编排、审批 UX 和 fallback 策略。

## 10. 被谁调用

- `createBaseToolRegistry().lookupHandler("computeruse.checkboxConfirm")`。
...
- 不要绕过 runtime guard 直接发真实 pointer event 或解析浏览器 DOM。
...
`computeruse.checkboxConfirm` 是底层能力承载面。TAP/agent 可以把它和光标定位、点击、移动、滚动、截图、`omniBase`、`mcpBase`、`shellBase` 组合，但这个 baseTool 自己不做策略编排。

完整 ToolSkill 调用手册放在 storage 文档：

`src/storagePool/baseToolStorage/computeruseBase/mouseEmulation/computeruse.checkboxConfirm/computeruse.checkboxConfirm.md`
