...
对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/mouseEmulation/computeruse.mouseScroll.ts`

## 1. 文件位置

- 所属模块：`agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/mouseEmulation`。
- 当前 entry：`computeruse.mouseScroll.ts`。
- storage 实现：`src/storagePool/baseToolStorage/computeruseBase/mouseEmulation/computeruse.mouseScroll/`。

## 2. 文件职责

公开鼠标滚动基础工具的 canonical storage 实现、handler、definition 和类型。
...
- `mouseScroll`：请求 runtime 执行一次受治理的指针滚轮动作。
...
- runtime 负责真实 OS wheel events、窗口焦点、坐标转换、权限、session handle 和清理。
...
- 导出 `mouseScrollHandler` 供 registry 挂载。
- 导出 `mouseScrollBaseToolDefinition` 供工具发现和描述使用。
- 导出 `planMouseScroll` / `executeMouseScroll` 供兼容测试和局部调用。
- 导出 public-safe 类型，支撑 runtime inspection 和 TAP 高级工具组合。

## 6. 输入边界

- `purpose`：滚动指针的目的。
- `target.deltaX` / `target.deltaY`：滚动向量，至少一个非零。
- `target.at`：可选滚动起点；不提供时由 runtime 在当前 cursor/focus 位置处理。
- `target.coordinateSpace`：坐标空间，支持 `screen` / `window` / `normalized`。
- `target.durationMs`：可选滚动耗时。
- `target.displayId` / `target.windowId`：可选 runtime target hints。
- `context.dryRun` / `context.guard` / scope 字段：执行边界和治理材料。
- `BaseToolInvokeRequest.runtimeId/sessionId/toolCallId`：handler 路径注入的运行态元数据。

## 7. 输出边界

- dry-run 返回 metadata-only pointer action envelope，并包含 normalized scroll target。
- 真实执行成功返回 `actionId`、runtime entry 和 audit metadata。
...
- 缺 runtimeId、purpose 或滚动向量返回 `MISSING_RUNTIME_ID` / `MISSING_PURPOSE` / `MISSING_SCROLL_DELTA`。
- 非法滚动向量、起点、duration、coordinateSpace、displayId 或 windowId 返回对应 invalid 错误。
...
- `BaseToolExecutorPort.computeruse.pointerAction`：唯一真实指针滚动入口。
- `runtime.governancePlane.toolInvocationGrant`：真实执行 guard 来源。
- `runtime.inputController.pointer`：OS wheel events、焦点、坐标转换和清理所有权。
- TAP 高级工具系统：负责编排、审批 UX 和 fallback 策略。

## 10. 被谁调用

- `createBaseToolRegistry().lookupHandler("computeruse.mouseScroll")`。
...
- 不要绕过 runtime guard 直接发真实 wheel event。

## 12. 最小实现建议

- entry 必须保持显式导出，不使用 bare `export *`。
- storage `core.ts` 负责 JSON 校验、scroll target normalization、dry-run、guard、provider missing/failure 和输出归一化。
...
`computeruse.mouseScroll` 是底层能力承载面。TAP/agent 可以把它和点击、移动、截图、`omniBase`、`mcpBase`、`shellBase` 组合，但这个 baseTool 自己不做策略编排。

完整 ToolSkill 调用手册放在 storage 文档：

`src/storagePool/baseToolStorage/computeruseBase/mouseEmulation/computeruse.mouseScroll/computeruse.mouseScroll.md`
...
- `Runtime Behavior` 必须写清真实副作用只走 `BaseToolExecutorPort.computeruse.pointerAction`，缺 runtime provider 返回 `PROVIDER_UNAVAILABLE`。
