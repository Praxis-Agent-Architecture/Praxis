...
对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/mouseEmulation/computeruse.mouseEmulation.ts`

## 1. 文件位置

- 所属模块：`agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/mouseEmulation`。
- 当前 entry：`computeruse.mouseEmulation.ts`。
- storage 实现：`src/storagePool/baseToolStorage/computeruseBase/mouseEmulation/computeruse.mouseEmulation/`。

## 2. 文件职责

公开鼠标操作序列基础工具的 canonical storage 实现、handler、definition 和类型。

这个 entry 文件只提供稳定导出面；鼠标步骤序列 contract、provider practice、runtime port 包装和结果归一化都在 storagePool 中完成。

## 2.1 文件名语义拆解

- `computeruse`：计算机使用基础工具家族。
- `mouseEmulation`：请求 runtime 执行一组受治理的鼠标 primitive，包括 cursor locate、move 和 click。
...
- runtime 负责真实 OS 指针事件、光标读取、窗口焦点、坐标转换、权限、session handle 和清理。
...
- 导出 `mouseEmulationHandler` 供 registry 挂载。
- 导出 `mouseEmulationBaseToolDefinition` 供工具发现和描述使用。
- 导出 `planMouseEmulation` / `executeMouseEmulation` 供兼容测试和局部调用。
- 导出 public-safe 类型，支撑 runtime inspection 和 TAP 高级工具组合。

## 6. 输入边界

- `purpose`：鼠标序列目的。
- `steps`：非空步骤数组，支持 `locate`、`move`、`click`。
- `maxSteps`：步骤数限制，默认 `16`。
- `context.dryRun` / `context.guard` / scope 字段：执行边界和治理材料。
- `BaseToolInvokeRequest.runtimeId/sessionId/toolCallId`：handler 路径注入的运行态元数据。

## 7. 输出边界

- dry-run 返回 metadata-only sequence envelope，并包含 normalized steps。
- 真实执行成功返回每个步骤的 `actionId` 或 cursor position、runtime entry 和 audit metadata。
- 输出不包含 provider 私有错误，不包含 TAP 策略判断，不包含 OS automation backend 细节。

## 8. 错误边界

- malformed JSON 返回 `INVALID_REQUEST` / `INVALID_CONTEXT`。
- 缺 steps、runtimeId 或 purpose 返回 `MISSING_STEPS` / `MISSING_RUNTIME_ID` / `MISSING_PURPOSE`。
- 非法步骤、坐标空间、坐标或点击次数返回对应 invalid 错误。
...
- `BaseToolExecutorPort.computeruse.locateCursor`：真实 cursor observation 入口。
- `BaseToolExecutorPort.computeruse.pointerAction`：真实 pointer move/click 入口。
- `runtime.governancePlane.toolInvocationGrant`：真实执行 guard 来源。
- `runtime.inputController.pointer`：OS 指针事件、光标读取、焦点、坐标转换和清理所有权。
- TAP 高级工具系统：负责编排、审批 UX 和 fallback 策略。

## 10. 被谁调用

- `createBaseToolRegistry().lookupHandler("computeruse.mouseEmulation")`。
...
- 不要绕过 runtime guard 直接发真实 pointer event 或读取 cursor state。

## 12. 最小实现建议

- entry 必须保持显式导出，不使用 bare `export *`。
- storage `core.ts` 负责 JSON 校验、step normalization、dry-run、guard、provider missing/failure 和输出归一化。
...
- storage operational 文档必须写明完整 handler 调用形状、runtime ports、错误码、返回 envelope 和 avoid-list。
...
`computeruse.mouseEmulation` 是底层能力承载面。TAP/agent 可以把它和截图、`omniBase`、`mcpBase`、`shellBase` 组合，但这个 baseTool 自己不做策略编排。

完整 ToolSkill 调用手册放在 storage 文档：

`src/storagePool/baseToolStorage/computeruseBase/mouseEmulation/computeruse.mouseEmulation/computeruse.mouseEmulation.md`
...
- `Runtime Behavior` 必须写清真实副作用只走 `BaseToolExecutorPort.computeruse.locateCursor` 和 `BaseToolExecutorPort.computeruse.pointerAction`，缺 runtime provider 返回 `PROVIDER_UNAVAILABLE`。
