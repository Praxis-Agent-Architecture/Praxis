...
对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/mouseEmulation/computeruse.mouseClick.ts`

## 1. 文件位置

- 所属模块：`agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/mouseEmulation`。
- 当前 entry：`computeruse.mouseClick.ts`。
- storage 实现：`src/storagePool/baseToolStorage/computeruseBase/mouseEmulation/computeruse.mouseClick/`。

## 2. 文件职责

公开鼠标点击基础工具的 canonical storage 实现、handler、definition 和类型。
...
- `mouseClick`：请求 runtime 执行一次受治理的指针点击动作。
...
- 导出 `mouseClickHandler` 供 registry 挂载。
- 导出 `mouseClickBaseToolDefinition` 供工具发现和描述使用。
- 导出 `planMouseClick` / `executeMouseClick` 供兼容测试和局部调用。
- 导出 public-safe 类型，支撑 runtime inspection 和 TAP 高级工具组合。

## 6. 输入边界

- `purpose`：点击目的。
- `target.button` / `target.clickCount`：点击按钮和次数。
- `target.at`：可选点击坐标；缺省时表示当前 cursor。
...
- 非法按钮、点击次数、坐标、displayId 或 windowId 返回对应 invalid 错误。
...
- `BaseToolExecutorPort.computeruse.pointerAction`：唯一真实指针点击入口。
...
- `createBaseToolRegistry().lookupHandler("computeruse.mouseClick")`。
...
`computeruse.mouseClick` 是底层能力承载面。TAP/agent 可以把它和截图、`omniBase`、`mcpBase`、`shellBase` 组合，但这个 baseTool 自己不做策略编排。

完整 ToolSkill 调用手册放在 storage 文档：

`src/storagePool/baseToolStorage/computeruseBase/mouseEmulation/computeruse.mouseClick/computeruse.mouseClick.md`
