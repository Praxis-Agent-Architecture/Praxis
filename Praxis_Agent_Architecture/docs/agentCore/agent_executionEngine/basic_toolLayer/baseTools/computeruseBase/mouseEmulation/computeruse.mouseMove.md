...
对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/mouseEmulation/computeruse.mouseMove.ts`

## 1. 文件位置

- 所属模块：`agent_executionEngine/basic_toolLayer/baseTools/computeruseBase/mouseEmulation`。
- 当前 entry：`computeruse.mouseMove.ts`。
- storage 实现：`src/storagePool/baseToolStorage/computeruseBase/mouseEmulation/computeruse.mouseMove/`。

## 2. 文件职责

公开鼠标移动基础工具的 canonical storage 实现、handler、definition 和类型。
...
- `mouseMove`：请求 runtime 执行一次受治理的指针移动动作。
...
- runtime 负责真实 OS 指针事件、窗口焦点、坐标转换、权限、session handle 和清理。
...
- 导出 `mouseMoveHandler` 供 registry 挂载。
- 导出 `mouseMoveBaseToolDefinition` 供工具发现和描述使用。
- 导出 `planMouseMove` / `executeMouseMove` 供兼容测试和局部调用。
- 导出 public-safe 类型，支撑 runtime inspection 和 TAP 高级工具组合。

## 6. 输入边界

- `purpose`：移动指针的目的。
- `target.x` / `target.y`：目标坐标。
- `target.coordinateSpace`：坐标空间，支持 `screen` / `window` / `normalized`。
- `target.durationMs`：可选移动耗时。
...
- dry-run 返回 metadata-only pointer action envelope，并包含 normalized target。
...
- 缺 runtimeId、purpose 或 target 坐标返回 `MISSING_RUNTIME_ID` / `MISSING_PURPOSE` / `MISSING_TARGET`。
- 非法坐标、duration、coordinateSpace、displayId 或 windowId 返回对应 invalid 错误。
...
- `BaseToolExecutorPort.computeruse.pointerAction`：唯一真实指针移动入口。
- `runtime.governancePlane.toolInvocationGrant`：真实执行 guard 来源。
- `runtime.inputController.pointer`：OS 指针事件、焦点、坐标转换和清理所有权。
- TAP 高级工具系统：负责编排、审批 UX 和 fallback 策略。

## 10. 被谁调用

- `createBaseToolRegistry().lookupHandler("computeruse.mouseMove")`。
...
- 不要绕过 runtime guard 直接发真实 pointer event。
...
`computeruse.mouseMove` 是底层能力承载面。TAP/agent 可以把它和点击、截图、`omniBase`、`mcpBase`、`shellBase` 组合，但这个 baseTool 自己不做策略编排。

完整 ToolSkill 调用手册放在 storage 文档：

`src/storagePool/baseToolStorage/computeruseBase/mouseEmulation/computeruse.mouseMove/computeruse.mouseMove.md`
