...
> 对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_executionEngine/basic_toolLayer/baseTools/searchBase/search.nativeSearch.ts`

## 1. 文件位置

- 所属顶层模块：执行引擎（`agent_executionEngine`）。
- 所属路径：`agent_executionEngine/basic_toolLayer/baseTools/searchBase`。
- 当前文件：`search.nativeSearch.ts`。
- 角色概括：Agent 的执行身体，负责输入输出、PromptPack、主循环、状态机、基础工具原语和执行事件暴露。

## 2. 文件职责

提供 基础工具集合 / 搜索基础工具 中的“三家官方原生网络搜索”基础能力原语。

这个文件的核心不是“占一个目录位置”，而是要在当前路径上形成一个可实现、可测试、可被 runtime 或相邻模块调用的窄能力点。它应该围绕“提供 基础工具集合 / 搜索基础工具 中的“三家官方原生网络搜索”基础能力原语”建立清晰的输入、输出、错误和治理边界。

## 2.1 文件名语义拆解

- 原始文件名：`search.nativeSearch.ts`。
- 命名片段：`search` / `native` / `Search`。
- 工程含义：这是 `searchBase` 下 `search.nativeSearch.ts` 分组里的 `nativeSearch` 基础工具原语，重点是把 OpenAI / Anthropic / DeepMind 官方内建网络搜索做成可治理、可审计、可测试的最小工具能力。
...
- TAP 治理、执行上下文、资源限制、provider 选择、网络搜索目标对象和审计上下文。
- `target.provider`、`target.query`、`target.model`、`target.maxResults`、`target.recencyDays` / `target.freshness`、`target.allowedDomains`、`target.searchContextSize`、`target.userLocation`、`target.citations`。
...
- 不泄漏底层实现细节的标准工具结果信封。
- 统一输出 `answer`、`sources`、`citations`、`providerMetadata` 和可选 `raw`，承接 OpenAI `web_search_call`、Anthropic `web_search`、Gemini `groundingMetadata` 等 provider 原生材料。

输出边界必须稳定：上层应该依赖这里给出的标准结构，而不是依赖内部临时变量、provider 原始字段或工具底层细节。

## 8. 错误边界

- 参数缺失、契约不满足、权限不足、provider 越权时必须返回可解释错误。
- 工具执行失败、环境缺失、网络搜索治理未通过、资源越界和审批未通过要区分处理。
...
- `BaseToolExecutorPort.network.nativeWebSearch`
- OpenAI / Anthropic / DeepMind 原生 web search provider adapter
- 基础环境、凭据托管与资源限制
...
- 不要在这里写上层产品逻辑，也不要让它直接绑定某一家 provider 的 SDK client 或网络请求格式。
- 不要把它做成本地文件搜索；本地搜索不属于这个语义。
- 不要把它和 `search.searchEngine`、`search.fetch`、`search.ground` 混用：前者是自建/通用搜索引擎，后两者分别是网页抓取和证据整理。
...
- 实现一个最小纯函数或薄类壳，能完成“提供 基础工具集合 / 搜索基础工具 中的“三家官方原生网络搜索”基础能力原语”的可测路径。
- 所有副作用先通过明确依赖注入进入，避免在文件内部偷偷读全局状态。
- 网络动作先只实现 dry-run / guard / audit path，再通过 runtime 注入的 `network.nativeWebSearch` 打开真实执行。
...
- 验证该文件确实只完成“提供 基础工具集合 / 搜索基础工具 中的“三家官方原生网络搜索”基础能力原语”，没有越界承担相邻模块职责。
- 验证错误结果可解释、可分类、不会泄漏不该暴露的内部细节。
- 验证 guard/dry-run/audit path，避免测试误触真实网络 provider。
