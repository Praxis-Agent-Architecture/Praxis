...
> 对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_executionEngine/basic_toolLayer/baseTools/gitBase/commit/git.createCommit.ts`

## 1. 文件位置

- 所属顶层模块：执行引擎（`agent_executionEngine`）。
- 所属路径：`agent_executionEngine/basic_toolLayer/baseTools/gitBase/commit`。
- 当前文件：`git.createCommit.ts`。
- storage 实现：`src/storagePool/baseToolStorage/gitBase/commit/git.createCommit/`。

## 2. 文件职责

提供 Git 基础工具 / 提交操作 中的“创建提交”基础能力原语。

它把创建提交做成固定动作工具：storage 负责 commit message 校验、固定 argv、风险和结果解析，runtime 只通过 `BaseToolExecutorPort.git.runGit` 执行 `{ repositoryPath, args, timeoutMs }`。这里是基础工具原语，不是 TAP 的最终高级工具库；TAP 仍负责更上层审批、组合和专业工具。

## 2.1 文件名语义拆解

- 原始文件名：`git.createCommit.ts`。
- 命名片段：`git` / `create` / `Commit`。
- 工程含义：这是 `gitBase` 下 `commit` 分组里的 `createCommit` 基础工具原语。
- 真实动作：固定为 `git commit`，不提供任意 `git.execute`。
...
- 通过统一 `BaseToolHandler.invoke()` 暴露 `git.createCommit`。
- 支持固定 `git commit [--all] [--allow-empty] [--signoff] -m <commitMessage>`。
- 返回 `runtimeEntry`、`risk`、`gitArgs`、`commandPreview`、`providerCalled`、`executionBlocked` 和 `resultEnvelope`。
- 保留 `planGitCommitCreation()` 兼容旧调用方。

## 6. 输入边界

- 输入必须是工具调用 JSON，主要字段是 `target.repositoryPath`、`target.commitMessage`、`includeAllTracked`、`allowEmpty`、`signoff`。
...
- 真实提交必须设置 `dryRun:false` 和 affirmative guard。

## 7. 输出边界

- 输出是可给 runtime inspection 和上层模型看的标准工具结果信封。
- 不泄漏 provider 栈、私有路径或 `.git` 内部细节。
- `resultEnvelope` 至少描述 commit message、operation hint、commit hash、branch、subject、files changed 和 commitCreated 状态。
...
- `createBaseToolRegistry().lookupHandler("git.createCommit")`。
- `scripts/agentCore_Agent_Test/agentcore_tool_lab.ts` 的 mounted gitBase runner。
- 后续 TAP 高级工具系统可以把它组合成“暂存 -> 提交 -> 检查状态”的完整工作流。

## 11. 不应该做什么

- 不要提供任意 `git.execute`。
- 不要改用 `shell.commandExecution` 执行提交。
- 不要把 amend/cherry-pick/revert/reset/merge/rebase/push 等动作塞进本工具。
- 不要让模型传入路径后自动成为 allowed repository root。

## 12. 最小实现建议

- entry 文件保持薄 re-export。
- storage `core.ts` 负责 JSON 验证、commit message 校验、固定 argv、dry-run、governance、provider error mapping 和 parser。
...
- fake runtime executor 收到固定 argv，例如 `commit --all --signoff -m Add file`。
