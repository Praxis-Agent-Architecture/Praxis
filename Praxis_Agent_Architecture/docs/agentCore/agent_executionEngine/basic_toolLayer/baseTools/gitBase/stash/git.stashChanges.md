...
> 对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_executionEngine/basic_toolLayer/baseTools/gitBase/stash/git.stashChanges.ts`
> 对应实现：`Praxis_Agent_Architecture/src/storagePool/baseToolStorage/gitBase/stash/git.stashChanges/`

## 1. 文件位置

- 所属顶层模块：执行引擎（`agent_executionEngine`）。
- 所属路径：`agent_executionEngine/basic_toolLayer/baseTools/gitBase/stash`。
- 当前入口文件：`git.stashChanges.ts`。
- storage 实现目录：`src/storagePool/baseToolStorage/gitBase/stash/git.stashChanges/`。
- runtime 入口：`BaseToolExecutorPort.git.runGit`。

## 2. 文件职责

`git.stashChanges` 提供 Git 基础工具 / stash 操作 中的“保存 stash”基础能力原语。

它的职责是把“保存当前工作区改动”做成 fixed-action baseTool：入口层只暴露稳定类型、planner、handler 和 definition；storage core 负责 JSON 校验、固定 argv、scope/permission/governance 检查、risk metadata、provider 调用和结果解析；runtime 负责真实本机 Git 进程。

该工具不是 `git.execute`。模型不能提供任意 Git 子命令，也不能把 stash 请求改走 `shell.commandExecution`。

## 2.1 文件名语义拆解

- `git`：所属 baseTool family。
- `stash`：Git stash 能力分组。
- `Changes`：保存当前工作区改动为 stash entry。
- 工程含义：上层通过统一 `BaseToolHandler.invoke()` 调用，底层只允许 `git stash push` 的固定动作。

## 3. 目录语义

- `baseTools/gitBase/stash/git.stashChanges.ts`：稳定公开入口，保持薄 re-export。
- `storagePool/baseToolStorage/gitBase/stash/git.stashChanges/core.ts`：契约、校验、固定 argv、治理、provider dispatch 和解析。
...
- `git.stashChanges.md`：operational toolSkill 文档。
...
- 支持保存当前工作区改动到 stash entry。
- 固定真实 Git argv：`git stash push [--include-untracked] [--keep-index] [-m message] [-- pathspec...]`。
- 支持 `message`、`includeUntracked`、`keepIndex` 和仓库相对 `pathspecs`。
...
- `target.pathspecs` 必须是仓库相对路径，禁止绝对路径、`..` 和 NUL。
- `target.message` 禁止 NUL。
...
- `resultEnvelope` 至少包含 message、includeUntracked、keepIndex、pathspecs、stdout/stderr 行数和 `createdStashHint`。
- 输出只暴露 public-safe 字段，不要求上层读取 provider 原始结构。

## 8. 错误边界

- `MISSING_REPOSITORY_PATH`：缺少仓库路径。
- `INVALID_CONTEXT` / `INVALID_MESSAGE` / `INVALID_PATHSPEC` / `INVALID_TIMEOUT`：输入形状错误。
- `PATHSPEC_OUTSIDE_SCOPE` / `SCOPE_REJECTED`：路径或仓库 scope 越界。
