...
> 对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_runtimeImplementation/runtime.execEngine/baseToolRuntimeGovernance.ts`
...
- `baseTool`：治理对象是现有 175 个 storage-backed BaseTool handler，不是 TAP 高级工具。
...
它位于 `ModelDecision -> BaseTool governance -> approval/session/event -> invokeMountedBaseTool -> observation material` 的中间层，让 175 个 BaseTool 从“能挂载”变成“能被 runtime 统一治理、解释、审计”。
