...
换句话说，`runtime-table` 不只是“有什么”，还要能说明“哪些决议已经生效，以及这些决议分别落在哪些正式边界上”。

这里冻结的是生效结果与最小必要归属，不是完整编译日志、推理链或审计解释文本。

### 4.6 外部官方模块接入位点

`runtime-table` 还应当承载与外部官方模块对接所需的装配落点信息。

这里说的接入位点，是指后续 `cmp / mp / tap / multiAgent` 等官方模块或外部治理系统，如何在宿主正式装配结果上定位自己的挂接面。

本文不定义这些模块的内部行为，也不提前写死它们的接入实现，只冻结宿主正式结果中必须存在可定位的装配落点。

这不等于把 `runtime-table` 写成模块注册表、模块配置中心或逐模块专用字段总表。
...
但本文只承认“存在接入位点这一层需求”，不把它写成固定的模块列表、固定字段，或逐模块配置注册表。
