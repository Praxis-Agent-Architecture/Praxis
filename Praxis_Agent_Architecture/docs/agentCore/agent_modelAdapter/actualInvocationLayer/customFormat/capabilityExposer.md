...
> 对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_modelAdapter/actualInvocationLayer/customFormat/capabilityExposer.ts`

## 1. 文件位置

- 所属顶层模块：模型适配层（`agent_modelAdapter`）。
- 所属路径：`agent_modelAdapter/actualInvocationLayer/customFormat`。
- 当前文件：`capabilityExposer.ts`。
- 角色概括：把 OpenAI、Anthropic、DeepMind/Gemini 和自定义上游格式接入 agentCore，并统一成内核可使用的模型能力。

## 2. 文件职责

把自定义上游格式中已经确认可用的能力暴露给模型适配层。

这个文件的核心不是“占一个目录位置”，而是要在当前路径上形成一个可实现、可测试、可被 runtime 或相邻模块调用的窄能力点。它应该围绕“把自定义上游格式中已经确认可用的能力暴露给模型适配层”建立清晰的输入、输出、错误和治理边界。

## 2.1 文件名语义拆解

- 原始文件名：`capabilityExposer.ts`。
- 命名片段：`capability` / `Exposer`。
- 工程含义：这是 `customFormat` 真实调用层中的 `capabilityExposer` 能力点，重点是承接上游现实形态。
- 第一实现重点：定义 provider 请求/响应/错误的受控包装，并把能力信号交给后续抽象层。
- 边界提醒：不要在 actualInvocationLayer 定义 agentCore 内部统一 AI 使用方式。

## 3. 目录语义

- 真实上游调用层：承接具体 provider 或自定义 endpoint 的真实调用形态。
- customFormat 调用面：处理 非官方、自定义或不兼容官方形态的上游格式 的请求、响应、错误与能力信号。
...
- 把自定义上游格式中已经确认可用的能力暴露给模型适配层
- 需要暴露的是经过整理后的能力，不是原始厂商或私有协议字段。
- 用于让 customFormat 和官方 provider 一样进入统一能力选择流程。
...
- 实现一个最小纯函数或薄类壳，能完成“把自定义上游格式中已经确认可用的能力暴露给模型适配层”的可测路径。
...
- 验证该文件确实只完成“把自定义上游格式中已经确认可用的能力暴露给模型适配层”，没有越界承担相邻模块职责。
