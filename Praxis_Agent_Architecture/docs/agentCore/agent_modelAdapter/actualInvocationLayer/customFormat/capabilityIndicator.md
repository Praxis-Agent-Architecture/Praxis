...
> 对应源码：`Praxis_Agent_Architecture/src/agentCore/agent_modelAdapter/actualInvocationLayer/customFormat/capabilityIndicator.ts`

## 1. 文件位置

- 所属顶层模块：模型适配层（`agent_modelAdapter`）。
- 所属路径：`agent_modelAdapter/actualInvocationLayer/customFormat`。
- 当前文件：`capabilityIndicator.ts`。
...
这个文件的核心不是“占一个目录位置”，而是要在当前路径上形成一个可实现、可测试、可被 runtime 或相邻模块调用的窄能力点。它应该围绕“标识自定义上游格式当前的能力可用信号”建立清晰的输入、输出、错误和治理边界。

## 2.1 文件名语义拆解

- 原始文件名：`capabilityIndicator.ts`。
- 命名片段：`capability` / `Indicator`。
- 工程含义：这是 `customFormat` 真实调用层中的 `capabilityIndicator` 能力点，重点是承接上游现实形态。
...
- 标识自定义上游格式当前的能力可用信号
- 需要承载可用、不可用、部分可用、需要鉴权、需要配置等状态。
- 给 abstractionLayer 和 runtime 检查面提供判断依据。
...
- 实现一个最小纯函数或薄类壳，能完成“标识自定义上游格式当前的能力可用信号”的可测路径。
...
- 验证该文件确实只完成“标识自定义上游格式当前的能力可用信号”，没有越界承担相邻模块职责。
