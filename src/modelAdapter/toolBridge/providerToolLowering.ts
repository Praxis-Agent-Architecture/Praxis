/*
 * 文件定位：Agent 模型适配层 / 新 toolBridge provider tool lowering。
 * 核心目的：把 Praxis harness tools lower 成 provider tool schema，并把 provider tool call raise 回 Praxis toolId。
 * 边界：只做 schema/transport 形状转换，不执行工具，不绕过 runtime governance。
 */
import { createHash } from "node:crypto";

export type RaxHarnessToolLike = {
  toolId: string;
  family?: string;
  group?: string;
  description?: string;
...
export type RaxManifestToolLike = {
  harness: {
    tools: readonly RaxHarnessToolLike[];
  };
};
...
function providerKindFor(tool: RaxHarnessToolLike): PraxisToolProviderKind {
...
function compareProviderTools(
  left: RaxHarnessToolLike,
  right: RaxHarnessToolLike,
): number {
...
function compactProviderToolDescription(tool: RaxHarnessToolLike): string {
