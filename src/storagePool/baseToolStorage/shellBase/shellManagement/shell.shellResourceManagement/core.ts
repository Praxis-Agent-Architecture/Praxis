...
function invocationId(context: ShellResourceManagementContext | undefined): string {
  return stringValue(context?.invocationId)?.trim() || "shell.shellResourceManagement:dry-run";
}
...
function normalizeAction(
  action: unknown,
  context: ShellResourceManagementContext | undefined,
): ShellResourceManagementAction | ShellResourceManagementResult {
  if (action === undefined) {
    return "inspect";
  }

  if (action === "inspect" || action === "reserve" || action === "release" || action === "adjust-limit") {
    return action;
  }

  return failure(
    "INVALID_ACTION",
    "shell.shellResourceManagement target.action is not supported",
    "input",
    context,
  );
}
...
function normalizeResourceId(
  ...
  const normalized = stringValue(resourceId)?.trim();
...
function normalizeTarget(
  ...
  const action = normalizeAction(target?.action, context);
  if (typeof action !== "string") {
    return action;
  }

  const resourceKindValue = target?.resourceKind;

  if (resourceKindValue === undefined) {
    return failure("MISSING_RESOURCE_KIND", "shell.shellResourceManagement requires target.resourceKind", "input", context);
  }

  if (typeof resourceKindValue !== "string") {
    return failure(
      "INVALID_RESOURCE_KIND",
  ...
  const resourceKind = resourceKindValue.trim() as ShellResourceKind;
  ...
  if (target?.limitName !== undefined && typeof target.limitName !== "string") {
    return failure("MISSING_LIMIT_NAME", "shell.shellResourceManagement adjust-limit requires limitName", "input", context, resourceId);
  }

  const limitName = stringValue(target?.limitName)?.trim();
...
function ensureGovernance(
  ...
  return failure(
    "GOVERNANCE_REJECTED",
    stringValue(context.guard.reason) ?? "shell.shellResourceManagement was rejected by runtime governance",
    "governance",
    context,
  );
}
