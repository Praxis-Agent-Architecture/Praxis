...
function invocationId(context: ShellSessionManagementContext | undefined): string {
  return stringValue(context?.invocationId)?.trim() || "shell.shellSessionManagement:dry-run";
}
...
function normalizeAction(
  action: unknown,
  context: ShellSessionManagementContext | undefined,
): ShellSessionManagementAction | ShellSessionManagementResult {
  if (action === undefined) {
    return "inspect";
  }

  if (action === "inspect" || action === "create" || action === "attach" || action === "detach" || action === "close") {
    return action;
  }

  return failure(
    "INVALID_ACTION",
    "shell.shellSessionManagement target.action is not supported",
    "input",
    context,
  );
}
...
function normalizeSingleLine(
  ...
  if (value !== undefined && typeof value !== "string") {
    return failure(errorCode, message, boundary, context);
  }
...
function ensureGovernance(
  ...
  return failure(
    "GOVERNANCE_REJECTED",
    stringValue(context.guard.reason) ?? "shell.shellSessionManagement was rejected by runtime governance",
    "governance",
    context,
  );
}
