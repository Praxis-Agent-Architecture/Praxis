...
export type ShellProcessTerminationOutput = {
  ...
  terminationEnvelope: Readonly<Record<string, unknown>> & {
...
function invocationId(context: ShellProcessTerminationContext | undefined): string {
  return trimmedString(readRecord(context)?.invocationId) || "shell.processTermination:dry-run";
}
...
function normalizeTarget(
  ...
  if (targetRecord?.processId === undefined) {
    return failure("MISSING_PROCESS_ID", "shell.processTermination requires target.processId", "input", context);
  }

  if (typeof targetRecord.processId !== "number" || !Number.isSafeInteger(targetRecord.processId) || targetRecord.processId <= 0) {
    return failure(
      "INVALID_PROCESS_ID",
      "shell.processTermination target.processId must be a positive integer",
      "input",
      context,
      typeof targetRecord.processId === "number" ? targetRecord.processId : undefined,
    );
  }

  const processId = targetRecord.processId as number;
  if (targetRecord.force !== undefined && typeof targetRecord.force !== "boolean") {
    return failure(
      "INVALID_FORCE",
      "shell.processTermination target.force must be boolean when provided",
      "input",
      context,
      processId,
    );
  ...
  if (targetRecord.signal !== undefined && typeof targetRecord.signal !== "string") {
    return failure(
      "UNSUPPORTED_SIGNAL",
  ...
  const signalValue = stringValue(targetRecord.signal) ?? (targetRecord.force === true ? "SIGKILL" : "SIGTERM");
  if (!supportedSignals.includes(signalValue as ShellProcessTerminationSignal)) {
    return failure(
      "UNSUPPORTED_SIGNAL",
      "shell.processTermination target.signal is not supported by this primitive",
      "input",
  ...
  const signal = signalValue as ShellProcessTerminationSignal;
  const reason = trimmedString(targetRecord.reason);
  if (targetRecord.reason !== undefined && reason === undefined) {
    return failure(
      "INVALID_REASON",
  ...
  return {
    ...
    signal,
    reason,
    force: targetRecord.force === true || signal === "SIGKILL",
  };
}
...
function ensureProcessScope(
  ...
  const allowedProcessIds = cleanNumberList(readRecord(context)?.allowedProcessIds);
...
function ensurePermissions(
  ...
  const grantedPermissions = cleanStringList(readRecord(context)?.grantedPermissions);
...
function ensureApproval(
  ...
  if (approval?.accepted === false) {
    return failure(
      "APPROVAL_REJECTED",
      stringValue(approval.reason) ?? "shell.processTermination approval was rejected by TAP governance",
      "approval",
      context,
      target.processId,
    );
