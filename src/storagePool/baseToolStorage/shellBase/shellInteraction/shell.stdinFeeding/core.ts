...
function invocationId(context: ShellStdinFeedingContext | undefined): string {
  return trimmedString(jsonRecord(context)?.invocationId) ?? "shell.stdinFeeding:dry-run";
}
...
function ensureScope(
  ...
  if (!allowedSessionIds.ok) {
    return failure("SCOPE_REJECTED", "shell.stdinFeeding allowedSessionIds must be a string array", "scope", context, sessionId);
  }
...
function normalizeTarget(
  ...
  const mode = (stringValue(targetRecord?.mode) ?? shellStdinFeedingDescriptor.defaultMode) as ShellStdinFeedingMode;
  ...
  const appendNewline = targetRecord?.appendNewline === true;
  const plannedInput = appendNewline ? `${input}\n` : input;
  const inputBytes = Buffer.byteLength(plannedInput, "utf8");
  const maxBytes = typeof context?.maxBytes === "number" ? context.maxBytes : shellStdinFeedingDescriptor.defaultMaxBytes;
  ...
  const requiresTapApproval = approvalRequired({ sensitive: targetRecord?.sensitive === true, mode });
  return {
    kind: "agentCore.basicTool.shell.stdinFeeding",
    sessionId,
    mode,
    appendNewline,
    inputPreview: previewInput(plannedInput, targetRecord?.sensitive === true),
    inputBytes,
...
export function planShellStdinFeeding(request: ShellStdinFeedingRequest = {}): ShellStdinFeedingResult {
  ...
  const context = jsonRecord(requestRecord.context) as ShellStdinFeedingContext | undefined;
  ...
  const approvalFailure = ensureApproval(output.requiresTapApproval, context, output.sessionId);
  ...
  return {
    ...
    output,
    audit: [
      auditEvent("agentCore.basicTool.shell.stdinFeeding.dryRun", context, output.sessionId, {
        mode: output.mode,
