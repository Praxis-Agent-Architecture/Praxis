---
description: prepare an audio transcription or lyric extraction request while leaving audio decoding and ASR to runtime.
...
Use `omni.audioLyricsGeneration` when an agent needs to prepare an audio transcription or lyric extraction request while leaving audio decoding and ASR to runtime. This baseTool is a contract and handler surface; it does not read media bytes, run codecs, upload files, poll jobs, choose a provider model, or lower provider-specific request bodies.
...
await handler.invoke({
  ...
  input: {
    ...
    context: {
      ...
      grantedPermissions: ["omni:audio:read", "provider:invoke"]
    }
  },
  executor
});
