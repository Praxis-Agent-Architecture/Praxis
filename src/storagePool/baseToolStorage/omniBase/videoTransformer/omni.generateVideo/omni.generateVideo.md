...
argument-hint: "{ target: { prompt, outputPath | outputRef, targetFormat?, durationSeconds? }, context?: { dryRun?, guard? } }"
---
...
Use `omni.generateVideo` when an agent needs to prepare a video generation request for provider-backed runtime execution. This baseTool is a contract and handler surface; it does not read media bytes, run codecs, upload files, poll jobs, choose a provider model, or lower provider-specific request bodies.
...
await handler.invoke({
  ...
  sessionId: "session-1",
  input: {
    target: { prompt: "A five second turntable shot of a small robot.", outputPath: "/workspace/output/clip.mp4", targetFormat: "mp4", durationSeconds: 5 },
    context: {
      dryRun: false,
      guard: { accepted: true, allowed: true },
      grantedPermissions: ["provider:invoke", "omni:video:generate", "omni:video:write"]
    }
  },
  executor
});
