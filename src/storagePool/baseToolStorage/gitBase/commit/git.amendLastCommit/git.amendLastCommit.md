---
description: "Amend the last commit through fixed git commit --amend semantics."
argument-hint: '{"target":{"repositoryPath":"/repo/project","commitMessage":"Refined","includeAllTracked":true,"resetAuthor":true},"context":{"dryRun":false,"guard":{"allowed":true,"accepted":true},"allowedRepositoryRoots":["/repo"],"grantedPermissions":["git:read","git:write","filesystem:read","filesystem:write"]}}'
---
...
Use `git.amendLastCommit` to amend the last commit through fixed git commit --amend semantics. It is a fixed-action gitBase tool exposed through the unified `BaseToolHandler.invoke()` surface. The model must call this narrow tool rather than `shell.commandExecution`, `git.execute`, or a made-up `gitBase.*` tool id.
...
- `target.commitMessage or target.noEdit`.
...
- `commit --amend [--all] [--reset-author] (--no-edit|-m <commitMessage>)`
...
- Do not bypass `createBaseToolRegistry().lookupHandler("git.amendLastCommit")` and `handler.invoke(...)` in integration tests.
