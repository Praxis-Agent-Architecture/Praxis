---
description: "Create a revert through fixed git revert semantics."
argument-hint: '{"target":{"repositoryPath":"/repo/project","commitRef":"deadbeef"},"context":{"dryRun":false,"guard":{"allowed":true,"accepted":true},"allowedRepositoryRoots":["/repo"],"grantedPermissions":["git:read","git:write","filesystem:read","filesystem:write"]}}'
---
...
Use `git.revertCommit` to create a revert through fixed git revert semantics. It is a fixed-action gitBase tool exposed through the unified `BaseToolHandler.invoke()` surface. The model must call this narrow tool rather than `shell.commandExecution`, `git.execute`, or a made-up `gitBase.*` tool id.
...
- `revert [--no-commit] [--mainline <n>] <commitRef>`
...
- Do not bypass `createBaseToolRegistry().lookupHandler("git.revertCommit")` and `handler.invoke(...)` in integration tests.
