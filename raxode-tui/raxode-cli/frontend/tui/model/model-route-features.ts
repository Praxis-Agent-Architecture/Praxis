import type { OpenAILiveConfig } from "../config/live-config.js";
import type { RaxodeReasoningEffort } from "../config/raxode-config.js";
import { isDeepSeekV4Model } from "@praxis-ai/praxis/modelAdapter/providerAccessLayer/modelMetadataRegistry";

type ProviderId = "openai" | "anthropic" | "deepmind";
...
export function parseProviderModelSelectionValue(
  ...
  if (routeKind === "openai_responses") {
    ...
    return {
      model: match[1].trim(),
      reasoning: match[2].trim() as RaxodeReasoningEffort,
