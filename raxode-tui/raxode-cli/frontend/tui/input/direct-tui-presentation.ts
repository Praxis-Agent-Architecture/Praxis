...
export function resolveDirectTuiContextUsedTokens(input: {
  ...
  const sessionContextTokens = finiteNonNegativeNumber(input.snapshot?.sessionContextTokens);
  ...
  return (sessionContextTokens ?? providerInputTokens ?? promptTokens ?? 0) + draftTokens;
}
...
function summarizeCodeToolPreview(input: {
  ...
  if (
    !input.toolId.startsWith("code.")
    && !input.toolId.startsWith("file.")
    && input.toolId !== "patch.apply"
  ) return undefined;
...
function resolveCodePreviewDiffStats(input: {
  ...
  if (!input.toolId.startsWith("code.") && input.toolId !== "patch.apply") return undefined;
  const args = flattenPreviewArguments(input.argumentsRecord);
  switch (input.toolId) {
    case "patch.apply": {
      const patch = previewStringValue(args.patch)
        ?? previewStringValue(args.content)
        ?? extractPreviewStringField(input.argumentsPreview, "patch")
...
function shellWorkspaceWriteLine(reason: string): string {
  return `${reason}; runtime will execute it under the active workspace policy.`;
}
...
function summarizeShellToolPreview(input: {
  ...
  if (command) {
    ...
    if (workspaceWriteReason !== undefined) {
      return shellWorkspaceWriteLine(workspaceWriteReason);
    }
...
function summarizeAgentToolPreview(input: {
  toolId: string;
  argumentsRecord?: Record<string, unknown>;
  argumentsPreview?: string | null;
}): string | undefined {
  if (!input.toolId.startsWith("agent.")) return undefined;
  const args = flattenPreviewArguments(input.argumentsRecord);
  const target = previewStringValue(args.target)
    ?? previewStringValue(args.agent)
    ?? previewStringValue(args.agentId)
    ?? extractPreviewStringField(input.argumentsPreview, "target")
    ?? extractPreviewStringField(input.argumentsPreview, "agent")
    ?? extractPreviewStringField(input.argumentsPreview, "agentId");
  const taskName = previewStringValue(args.task_name)
    ?? previewStringValue(args.taskName)
    ?? previewStringValue(args.name)
    ?? extractPreviewStringField(input.argumentsPreview, "task_name")
    ?? extractPreviewStringField(input.argumentsPreview, "taskName")
    ?? extractPreviewStringField(input.argumentsPreview, "name");
  const agentType = previewStringValue(args.agent_type)
    ?? previewStringValue(args.agentType)
    ?? extractPreviewStringField(input.argumentsPreview, "agent_type")
    ?? extractPreviewStringField(input.argumentsPreview, "agentType");
  const messageId = previewStringValue(args.messageId)
    ?? previewStringValue(args.message_id)
    ?? extractPreviewStringField(input.argumentsPreview, "messageId")
    ?? extractPreviewStringField(input.argumentsPreview, "message_id");
  const message = previewStringValue(args.message)
    ?? previewStringValue(args.prompt)
    ?? extractPreviewStringField(input.argumentsPreview, "message")
    ?? extractPreviewStringField(input.argumentsPreview, "prompt");
  const timeoutMs = previewNumberValue(args.timeoutMs)
    ?? previewNumberValue(args.timeout_ms)
    ?? extractPreviewNumberField(input.argumentsPreview, "timeoutMs")
    ?? extractPreviewNumberField(input.argumentsPreview, "timeout_ms");
  const timeoutSeconds = timeoutMs === undefined
    ? undefined
    : Math.max(0, Math.round(timeoutMs / 1000));

  switch (input.toolId) {
    case "agent.spawn":
      return [
        `Spawning ${taskName ?? agentType ?? "agent"}`,
        message ? `for ${truncatePreviewText(message, 120)}` : undefined,
      ].filter((part): part is string => part !== undefined).join(" ");
    case "agent.message":
      return `Messaging ${target ?? "agent"}${message ? `: ${truncatePreviewText(message, 140)}` : ""}`;
    case "agent.wait":
      return [
        `Waiting for ${target ?? messageId ?? "agent result"}`,
        timeoutSeconds !== undefined ? `(${timeoutSeconds}s timeout)` : undefined,
      ].filter((part): part is string => part !== undefined).join(" ");
    default:
      return `${input.toolId}${target ? ` ${target}` : ""}${message ? `: ${truncatePreviewText(message, 140)}` : ""}`;
  }
}
...
export function resolveDirectTuiToolPreviewSummaryLines(input: {
  ...
  const shellWorkspaceWriteDetected = shellWorkspaceWriteReason !== undefined;
  ...
  const actionLine = procedureLines?.[0]
    ?? (shellWorkspaceWriteReason !== undefined ? shellWorkspaceWriteLine(shellWorkspaceWriteReason) : undefined)
    ...
    ?? summarizeAgentToolPreview({ toolId, argumentsRecord, argumentsPreview: input.argumentsPreview })
    ?? summarizeGenericToolPreview({
      providerToolName: input.providerToolName,
      argumentsRecord,
      argumentsPreview: input.argumentsPreview,
    });
  const displayTitle = shellWorkspaceWriteDetected && title === "Tool" ? "Shell" : title;
  const displayPhase = phase === "done"
      ? "ready"
      : "composing";
  return [
...
export function resolveDirectTuiProcedurePlannedToolPreviews(input: {
  ...
  const codeToolIds = uniqueToolIds.filter((plannedToolId) =>
    plannedToolId.startsWith("code.")
    || plannedToolId.startsWith("file.")
    || plannedToolId === "patch.apply");
...
export function resolveDirectTuiToolSummaryResultLineLimit(input: {
  ...
  if (familyKey === "plan" || familyKey === "workflow") {
    return 8;
  }
