...
function isPlannedToolPreviewMessage(message: SurfaceMessage): boolean {
  return isToolPreviewMessage(message) && message.metadata?.plannedFromProcedure === true;
}
...
function toolSummaryCallVisibilityKey(message: SurfaceMessage): string | undefined {
  const turnId = message.turnId?.trim();
  const resultMetadata = message.metadata?.resultMetadata;
  const resultToolCallId = resultMetadata && typeof resultMetadata === "object" && !Array.isArray(resultMetadata)
    ? (resultMetadata as Record<string, unknown>).toolCallId
    : undefined;
  const toolCallId = typeof message.metadata?.toolCallId === "string" && message.metadata.toolCallId.trim()
    ? message.metadata.toolCallId.trim()
    : (typeof resultToolCallId === "string" ? resultToolCallId.trim() : "");
  return turnId && toolCallId ? `${turnId}:call:${toolCallId}` : undefined;
}
...
function filterSupersededToolPreviewMessages(messages: readonly SurfaceMessage[]): SurfaceMessage[] {
  ...
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    ...
    const callVisibilityKey = toolSummaryCallVisibilityKey(message);
    const familyKey = toolSummaryFamilyKey(message);
    if (
      isToolPreviewMessage(message)
      && (
        (callVisibilityKey !== undefined && laterFinalSummaryKeys.has(callVisibilityKey))
        ||
        (visibilityKey !== undefined && laterFinalSummaryKeys.has(visibilityKey))
        || (isPlannedToolPreviewMessage(message) && turnVisibilityKey !== undefined && laterFinalSummaryKeys.has(turnVisibilityKey))
    ...
    if (isFinalToolSummaryMessage(message) && visibilityKey) {
      ...
      if (callVisibilityKey) {
        laterFinalSummaryKeys.add(callVisibilityKey);
      }
