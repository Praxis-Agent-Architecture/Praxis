...
import type { RaxodeReadinessDigest } from "../../bridge/readiness.js";
import { loadOpenAILiveConfig, type OpenAILiveConfig } from "../config/live-config.js";
import {
  loadRaxodeConfigFile as loadRaxodeConfigFile,
  loadRaxodeRuntimeConfigSnapshot as loadRaxodeRuntimeConfigSnapshot,
  loadResolvedEmbeddingConfig,
  loadResolvedRoleConfig,
  resolveConfiguredWorkspaceRoot,
  type RaxodeAnimationMode as RaxodeAnimationMode,
  type RaxodeConfigFile as RaxodeConfigFile,
  type RaxodeRoleId,
  type RaxodeReasoningEffort as RaxodeReasoningEffort,
  writeRaxodeConfigFile as writeRaxodeConfigFile,
} from "../config/raxode-config.js";
import { getOpenAIAuthStatus } from "../auth/openai-auth.js";
import { resolveAppRoot } from "../runtime/runtime-paths.js";
import {
...
} from "../input/composer-popup-pagination.js";
import {
  listDirectTuiAgents,
...
} from "../input/direct-turn-checkpoints.js";
import {
...
} from "../input/status-rate-limits.js";
import {
...
} from "../input/rewind-state.js";
import { resolveLastKnownTurnIndex, resolveNextOptimisticTurnIndex } from "../input/optimistic-turn-index.js";
import {
  formatHumanGateDecisionEnvelope,
} from "../approval/human-gate-envelope.js";
import {
...
} from "../model/model-route-features.js";
import {
...
} from "../input/viewer-status-blocks.js";
import { buildCapabilityViewerBodyLines } from "../input/capability-viewer-panel.js";
import {
  resolveCapabilityFamilyDefinition,
  resolveFamilyOutcomeKind,
  shouldRenderCapabilityFamilyBlock,
} from "../telemetry/family-telemetry.js";
import {
...
} from "../input/direct-input-helpers.js";
import {
  refineWebSearchToolSummary,
  summarizePendingComposerText,
} from "../summary/tui-mini-summary.js";
import { enableTerminalMouseReporting, parseTerminalMouseEvents } from "../terminal/mouse.js";
import {
  consumeBracketedPasteInput,
  enableTerminalBracketedPaste,
  isTerminalPasteShortcutInput,
} from "../terminal/paste.js";
import {
  extractSelectedText,
  finishTextSelection,
...
} from "../input/workspace-git-checkpoint.js";
import { restoreWorkspaceGitCheckpointInSubprocess } from "../input/workspace-git-checkpoint-subprocess.js";
import {
...
const RAXODE_RENDER_FPS = resolvePositiveFps(process.env.RAXODE_RENDER_FPS, 30);
const ANIMATION_TICK_MS = Math.max(1, 1000 / RAXODE_RENDER_FPS);
const RUN_STATUS_RENDER_FPS = resolvePositiveFps(process.env.RAXODE_RUN_STATUS_FPS, 30);
const RUN_STATUS_ANIMATION_TICK_MS = Math.max(1, 1000 / RUN_STATUS_RENDER_FPS);
const TOOL_SUMMARY_ANIMATION_TICK_MS = Math.max(
  80,
  1000 / resolvePositiveFps(process.env.RAXODE_TOOL_SUMMARY_FPS, 8),
);
const ANIMATION_SPEED_MULTIPLIER = 3.45;
...
const LOG_TAIL_READ_CHUNK_BYTES = 16 * 1024;
const LOG_TAIL_MAX_BYTES_PER_TICK = 64 * 1024;
const LOG_TAIL_PROCESS_BATCH_SIZE = 20;
...
const STREAMING_ASSISTANT_LIVE_MAX_LINES = 80;
const STREAMING_ASSISTANT_LIVE_MAX_CHARS = 8_000;
...
const ASSISTANT_DELTA_RENDER_INTERVAL_MS = Math.max(
  8,
  1000 / Math.max(1, Number.isFinite(Number.parseFloat(process.env.RAXODE_ASSISTANT_DELTA_FPS ?? "12"))
    ? Number.parseFloat(process.env.RAXODE_ASSISTANT_DELTA_FPS ?? "12")
    : 12),
);
...
interface SlashPanelContext {
  ...
  backendReadinessDigest: RaxodeReadinessDigest | null;
}
...
function resolveModelFieldRoleId(fieldKey: string): RaxodeRoleId | null {
  switch (fieldKey) {
    ...
    case "status":
      return {
        ...
        bodyLines: [
          ...
          createBodyKeyValueLine("Praxis backend modules:", formatBackendModuleStatusLine(context.backendReadinessDigest), { indent: 4, labelWidth: 28 }),
          createBodyKeyValueLine("Backend module gaps:", formatBackendModuleGapsLine(context.backendReadinessDigest), { indent: 4, labelWidth: 28 }),
        ...
        fields: [
          ...
          },
          {
            kind: "value",
            key: "backendModules",
            label: "Praxis backend modules",
            value: formatBackendModuleStatusLine(context.backendReadinessDigest),
          },
          {
            kind: "action",
            key: "showLogPath",
...
function isPlanUpdateCapability(capabilityKey?: string | null): boolean {
  return capabilityKey?.trim().toLowerCase() === "plan.update";
}

function extractPlanUpdateJson(source?: string | null): string | undefined {
...
function parsePlanUpdatePayload(record: LiveLogRecord): ParsedPlanUpdate | undefined {
  const source = extractPlanUpdateJson(record.argumentsPreview)
    ?? extractPlanUpdateJson(record.arguments)
    ?? extractPlanUpdateJson(record.inputSummary)
    ?? extractPlanUpdateJson(typeof record.resultMetadata?.inputSummary === "string" ? record.resultMetadata.inputSummary : undefined);
  if (!source) {
    return undefined;
  }
  try {
    ...
    const steps = plan.flatMap((item) => {
      if (!item || typeof item !== "object" || Array.isArray(item)) {
        return [];
      }
      const recordItem = item as Record<string, unknown>;
      const step = typeof recordItem.step === "string" ? compactRuntimeText(recordItem.step) : "";
      if (!step) {
        return [];
      }
      const status = typeof recordItem.status === "string" ? recordItem.status.trim().toLowerCase() : undefined;
      return [{ step, status }];
    });
    const explanation = typeof payload.explanation === "string" && payload.explanation.trim()
      ? compactRuntimeText(payload.explanation)
      : undefined;
    return explanation || steps.length > 0 ? { explanation, steps } : undefined;
  } catch {
    return undefined;
  }
}
...
function planStatusLabel(status?: string): string {
  switch (status) {
    ...
    case "todo":
      return "todo";
    default:
      return status ? compactRuntimeText(status) : "todo";
  }
}

function resolvePlanUpdateIntentLine(record: LiveLogRecord): string | null {
  const parsed = parsePlanUpdatePayload(record);
  return parsed?.explanation ? `Plan: ${parsed.explanation}` : "Plan updated";
}

function resolvePlanUpdateResultLines(record: LiveLogRecord): string[] | null {
  const parsed = parsePlanUpdatePayload(record);
  if (!parsed || parsed.steps.length === 0) {
    return null;
  }
  const visibleSteps = parsed.steps.slice(0, 8).map((item) => `${planStatusLabel(item.status)}: ${item.step}`);
  if (parsed.steps.length > visibleSteps.length) {
    visibleSteps.push(`... ${parsed.steps.length - visibleSteps.length} more steps`);
  }
  return visibleSteps;
}

function parseRuntimeJsonObject(source?: string | null): Record<string, unknown> | undefined {
  const trimmed = source?.trim();
...
function decodeRuntimeJsonStringFragment(value: string): string {
  try {
...
function extractRuntimeStringField(source: string | undefined | null, field: string): string | undefined {
  const text = source?.trim();
  if (!text) return undefined;
  const match = text.match(new RegExp(`"${field}"\\s*:\\s*"((?:\\\\.|[^"\\\\])*)"`, "u"))?.[1];
  return match === undefined ? undefined : compactRuntimeText(decodeRuntimeJsonStringFragment(match));
}

function extractRuntimeNumberField(source: string | undefined | null, field: string): number | undefined {
  const text = source?.trim();
  if (!text) return undefined;
...
function runtimeStringField(record: Record<string, unknown> | undefined, key: string): string | undefined {
  const value = record?.[key];
  return typeof value === "string" && value.trim().length > 0 ? compactRuntimeText(value) : undefined;
}

function runtimeNumberField(record: Record<string, unknown> | undefined, key: string): number | undefined {
  const value = record?.[key];
  return typeof value === "number" && Number.isFinite(value) ? value : undefined;
}

function agentArgumentRecord(record: LiveLogRecord): Record<string, unknown> | undefined {
  return parseRuntimeJsonObject(record.argumentsPreview)
    ?? parseRuntimeJsonObject(record.arguments)
    ?? parseRuntimeJsonObject(record.inputSummary)
    ?? parseRuntimeJsonObject(typeof record.resultMetadata?.inputSummary === "string" ? record.resultMetadata.inputSummary : undefined);
}

function agentStringArgument(record: LiveLogRecord, ...keys: string[]): string | undefined {
  const parsed = agentArgumentRecord(record);
  for (const key of keys) {
    const value = runtimeStringField(parsed, key)
      ?? extractRuntimeStringField(record.argumentsPreview, key)
      ?? extractRuntimeStringField(record.arguments, key)
      ?? extractRuntimeStringField(record.inputSummary, key);
    if (value !== undefined) return value;
  }
  return undefined;
}

function agentNumberArgument(record: LiveLogRecord, ...keys: string[]): number | undefined {
  const parsed = agentArgumentRecord(record);
  for (const key of keys) {
    const value = runtimeNumberField(parsed, key)
      ?? extractRuntimeNumberField(record.argumentsPreview, key)
      ?? extractRuntimeNumberField(record.arguments, key)
      ?? extractRuntimeNumberField(record.inputSummary, key);
    if (value !== undefined) return value;
  }
  return undefined;
}

function formatAgentIntentLine(record: LiveLogRecord): string | null {
  const capabilityKey = record.capabilityKey?.trim().toLowerCase();
  if (!capabilityKey?.startsWith("agent.")) {
    return null;
  }
  const target = agentStringArgument(record, "target", "agent", "agentId");
  const taskName = agentStringArgument(record, "task_name", "taskName", "name");
  const agentType = agentStringArgument(record, "agent_type", "agentType");
  const description = agentStringArgument(record, "description");
  const message = agentStringArgument(record, "message", "prompt");
  const messageId = agentStringArgument(record, "messageId", "message_id");
  const timeoutMs = agentNumberArgument(record, "timeoutMs", "timeout_ms");
  const timeoutSeconds = timeoutMs === undefined ? undefined : Math.max(0, Math.round(timeoutMs / 1000));

  switch (capabilityKey) {
    case "agent.spawn":
      return [
        `Spawning ${taskName ?? agentType ?? "agent"}`,
        description ? `for ${description}` : message ? `for ${message}` : undefined,
      ].filter((part): part is string => part !== undefined).join(" ");
    case "agent.message":
      return `Messaging ${target ?? "agent"}${message ? `: ${message}` : ""}`;
    case "agent.wait":
      return [
        `Waiting for ${target ?? messageId ?? "agent reply"}`,
        timeoutSeconds !== undefined ? `(${timeoutSeconds}s timeout)` : undefined,
      ].filter((part): part is string => part !== undefined).join(" ");
    default:
      return `${record.capabilityKey}${target ? ` ${target}` : ""}${message ? `: ${message}` : ""}`;
  }
}
...
function isFileReadCapability(capabilityKey?: string | null): boolean {
  const normalized = capabilityKey?.trim().toLowerCase();
  return normalized === "file.read" || normalized === "code.read";
}

function isShellRunCapability(capabilityKey?: string | null): boolean {
  return capabilityKey?.trim().toLowerCase() === "shell.run";
}

function commandFromShellSummaryLine(line: string): string | undefined {
  const compacted = compactRuntimeText(line);
  const prefixes = [
    "Running ",
  ...
  for (const prefix of prefixes) {
    if (compacted.startsWith(prefix)) {
      return compacted.slice(prefix.length).trim();
    }
  }
  return undefined;
}

function compactShellResultLines(record: LiveLogRecord, resultLines: string[], metadataLines: string[]): string[] {
  if (!isShellRunCapability(record.capabilityKey)) {
    return [...resultLines, ...metadataLines];
  }
  const command = resultLines.map(commandFromShellSummaryLine).find((value): value is string => value !== undefined);
  const statusLine = resultLines.find((line) => /^Command (?:completed|failed)/u.test(compactRuntimeText(line)));
  const outputLines = resultLines.filter((line) => /^(?:stdout|stderr):/u.test(compactRuntimeText(line)));
  const otherLines = resultLines.filter((line) => {
    const parsedCommand = commandFromShellSummaryLine(line);
    return (parsedCommand === undefined || parsedCommand !== command)
      && line !== statusLine
      && !/^(?:stdout|stderr):/u.test(compactRuntimeText(line));
  });
  return [
    ...
    ...otherLines,
    ...metadataLines,
  ].filter((line): line is string => typeof line === "string" && line.trim().length > 0).slice(0, 4);
}

function resolveGenericFamilyIntentLine(record: LiveLogRecord): string | null {
  if (isPlanUpdateCapability(record.capabilityKey)) {
    return resolvePlanUpdateIntentLine(record);
  }
  if (isFileReadCapability(record.capabilityKey)) {
    return null;
  }
  const agentIntentLine = formatAgentIntentLine(record);
  if (agentIntentLine) {
    return agentIntentLine;
  }
  if (typeof record.familyIntentSummary === "string" && record.familyIntentSummary.trim()) {
    return compactRuntimeText(record.familyIntentSummary);
  }
...
function resolveGenericFamilyResultLines(record: LiveLogRecord): string[] {
  if (isPlanUpdateCapability(record.capabilityKey)) {
    const planLines = resolvePlanUpdateResultLines(record);
    if (planLines) {
      return planLines;
    }
  }
  if (isFileReadCapability(record.capabilityKey)) {
    const readLine = Array.isArray(record.familyResultSummary)
      ? record.familyResultSummary
        .filter((line): line is string => typeof line === "string" && line.trim().length > 0)
        .find((line) => compactRuntimeText(line).startsWith("Read "))
      : undefined;
    if (readLine) {
      return [compactRuntimeText(readLine)];
    }
    if (typeof record.inputSummary === "string" && record.inputSummary.trim()) {
      return [compactRuntimeText(record.inputSummary).replace(/^Reading /u, "Read ")];
    }
  }
  if (Array.isArray(record.familyResultSummary)) {
    const normalized = record.familyResultSummary
...
function compactGenericFamilyResultLines(record: LiveLogRecord, resultLines: string[], metadataLines: string[]): string[] {
  ...
  return compactShellResultLines(record, compactedResultLines, metadataLines);
}
...
function resolveGenericFamilyMetadataLines(
  ...
  const requestKind = typeof metadataRecord.requestKind === "string"
    ...
    : undefined;
  const messageId = typeof metadataRecord.messageId === "string"
    ? compactRuntimeText(metadataRecord.messageId as string)
    : typeof metadataRecord.message_id === "string"
      ? compactRuntimeText(metadataRecord.message_id as string)
      : agentStringArgument(record, "messageId", "message_id");
  const agentName = typeof metadataRecord.agentName === "string"
    ? compactRuntimeText(metadataRecord.agentName as string)
    : typeof metadataRecord.taskName === "string"
      ? compactRuntimeText(metadataRecord.taskName as string)
      : agentStringArgument(record, "task_name", "taskName", "name");
  const agentTarget = typeof metadataRecord.target === "string"
    ? compactRuntimeText(metadataRecord.target as string)
    : typeof metadataRecord.agentId === "string"
      ? compactRuntimeText(metadataRecord.agentId as string)
      : agentStringArgument(record, "target", "agent", "agentId");
  ...
  if (phase === "start") {
    switch (familyKey) {
      ...
      case "agent":
        ...
        if (agentTarget) {
          lines.push(`Target: ${agentTarget}`);
        }
        if (messageId) {
          lines.push(`Message: ${messageId.slice(0, 24)}`);
        }
        break;
      case "useract":
        if (requestKind) {
...
function normalizeContextSnapshot(
  ...
  const sessionContextTokens = typeof input.sessionContextTokens === "number" && Number.isFinite(input.sessionContextTokens)
    ? input.sessionContextTokens
    : undefined;
  const compressionLimitTokens = typeof input.compressionLimitTokens === "number" && Number.isFinite(input.compressionLimitTokens)
    ? input.compressionLimitTokens
    : undefined;
  ...
  const lastRequestTotalTokens = typeof input.lastRequestTotalTokens === "number" && Number.isFinite(input.lastRequestTotalTokens)
    ...
    : undefined;
  const promptPackTokens = typeof input.promptPackTokens === "number" && Number.isFinite(input.promptPackTokens)
    ? input.promptPackTokens
    : undefined;
...
function formatStatusContextUsageLineFromSnapshot(input: {
  snapshot: ReturnType<typeof normalizeContextSnapshot>;
  used: number;
  total: number;
}): string {
  const base = formatStatusContextUsageLine(input.used, input.total);
  const lastInputTokens = input.snapshot?.lastRequestInputTokens;
  const promptPackTokens = input.snapshot?.promptPackTokens;
  const details = [
    lastInputTokens === undefined ? undefined : `input ${formatContextWindowLabel(lastInputTokens)}`,
    promptPackTokens === undefined || Math.abs(promptPackTokens - input.used) < 500
      ? undefined
      : `pack ${formatContextWindowLabel(promptPackTokens)}`,
  ].filter((detail): detail is string => detail !== undefined);
  return `${base} session${details.length === 0 ? "" : ` · ${details.join(" · ")}`}`;
}
...
function PraxisDirectTuiApp(): JSX.Element {
  ...
  const [backendReadinessDigest, setBackendReadinessDigest] = useState<RaxodeReadinessDigest | null>(null);
  ...
  const activeAssistantSegmentStartLengthRef = useRef(new Map<string, number>());
  const activeAssistantSegmentPreviewTextRef = useRef(new Map<string, string>());
  ...
  const pendingRawAssistantDeltaChunksRef = useRef(new Map<string, string[]>());
  ...
  const pendingToolPreviewArgumentChunksRef = useRef(new Map<string, string[]>());
  ...
  const surfaceStateFlushTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const assistantDeltaRenderCoalescerRef = useRef(createDirectTuiAssistantStreamCoalescer<{
    ...
    emit: (update) => {
      const baseLength = rawAssistantDeltaTextRef.current.get(update.turnId)?.length ?? 0;
      const deltaText = consumePendingAssistantDeltaText(update.turnId);
      if (!deltaText) {
        return;
      }
      emitAssistantDeltaChunkUpdate({
        turnId: update.turnId,
        at: update.payload.at,
        deltaText,
        segmentStartLength: baseLength,
      });
    },
  }));
  ...
  const flushPendingSurfaceState = () => {
    ...
    surfaceStateFlushTimerRef.current = null;
  ...
  const scheduleSurfaceStateFlush = () => {
    ...
    surfaceStateFlushTimerRef.current = setTimeout(() => {
      flushPendingSurfaceState();
    }, 0);
  };

  useEffect(() => () => {
    if (surfaceStateFlushTimerRef.current) {
      clearTimeout(surfaceStateFlushTimerRef.current);
      surfaceStateFlushTimerRef.current = null;
    }
  }, []);
  ...
  const loadChatModelCatalog = async (roleId: RaxodeRoleId = "core.main") => {
  ...
  const startAssistantSegment = (turnId: string, segmentStartLength?: number): string => {
    ...
    activeAssistantSegmentStartLengthRef.current.set(
      turnId,
      segmentStartLength ?? emittedAssistantTextRef.current.get(turnId)?.length ?? 0,
    );
    activeAssistantSegmentPreviewTextRef.current.delete(turnId);
    return messageId;
  };
  ...
  const appendRawAssistantDeltaTextChunk = (turnId: string, text: string) => {
    const decodedText = decodeEscapedDisplayTextMaybe(text);
    const chunks = pendingRawAssistantDeltaChunksRef.current.get(turnId);
    if (chunks) {
      chunks.push(decodedText);
      return;
    }
    pendingRawAssistantDeltaChunksRef.current.set(turnId, [decodedText]);
  };

  const materializeRawAssistantDeltaText = (turnId: string): string => {
    const chunks = pendingRawAssistantDeltaChunksRef.current.get(turnId);
    const baseText = rawAssistantDeltaTextRef.current.get(turnId) ?? "";
    if (!chunks || chunks.length === 0) {
      return baseText;
    }
    const nextText = `${baseText}${chunks.join("")}`;
    pendingRawAssistantDeltaChunksRef.current.delete(turnId);
    rawAssistantDeltaTextRef.current.set(turnId, nextText);
    return nextText;
  };

  const consumePendingAssistantDeltaText = (turnId: string): string => {
    const chunks = pendingRawAssistantDeltaChunksRef.current.get(turnId);
    if (!chunks || chunks.length === 0) {
      return "";
    }
    const deltaText = chunks.join("");
    pendingRawAssistantDeltaChunksRef.current.delete(turnId);
    rawAssistantDeltaTextRef.current.set(turnId, `${rawAssistantDeltaTextRef.current.get(turnId) ?? ""}${deltaText}`);
    return deltaText;
  };

  const setRawAssistantDeltaText = (turnId: string, text: string) => {
    pendingRawAssistantDeltaChunksRef.current.delete(turnId);
    rawAssistantDeltaTextRef.current.set(turnId, text);
  };

  const deleteRawAssistantDeltaText = (turnId: string) => {
    pendingRawAssistantDeltaChunksRef.current.delete(turnId);
    rawAssistantDeltaTextRef.current.delete(turnId);
  };

  const clearRawAssistantDeltaText = () => {
    pendingRawAssistantDeltaChunksRef.current.clear();
    rawAssistantDeltaTextRef.current.clear();
  };

  const appendToolPreviewArgumentChunk = (stateKey: string, chunk: unknown) => {
    if (typeof chunk !== "string" || chunk.length === 0) {
      return;
    }
    const chunks = pendingToolPreviewArgumentChunksRef.current.get(stateKey);
    if (chunks) {
      chunks.push(chunk);
      return;
    }
    pendingToolPreviewArgumentChunksRef.current.set(stateKey, [chunk]);
  };

  const materializeToolPreviewArguments = (stateKey: string, baseText: string): string => {
    const chunks = pendingToolPreviewArgumentChunksRef.current.get(stateKey);
    if (!chunks || chunks.length === 0) {
      return baseText;
    }
    const nextText = `${baseText}${chunks.join("")}`;
    pendingToolPreviewArgumentChunksRef.current.delete(stateKey);
    return nextText;
  };

  const buildStreamingAssistantDisplayText = (text: string): string => {
    const tail = tailStreamingAssistantText(text);
    return tail.trimmed
      ? `[showing recent output]\n${tail.text}`
      : tail.text;
  };

  const appendStreamingAssistantPreviewText = (turnId: string, deltaText: string): string => {
    const previousText = activeAssistantSegmentPreviewTextRef.current.get(turnId) ?? "";
    const nextTail = tailStreamingAssistantText(`${previousText}${deltaText}`);
    const nextText = nextTail.text;
    activeAssistantSegmentPreviewTextRef.current.set(turnId, nextText);
    return buildStreamingAssistantDisplayText(nextText);
  };

  const emitAssistantDeltaChunkUpdate = (input: {
    turnId: string;
    at: string;
    deltaText: string;
    segmentStartLength: number;
  }) => {
    applyAssistantIntentRunIndicator(input.deltaText);
    const activeAssistantMessageId = activeAssistantMessageIdRef.current.get(input.turnId)
      ?? startAssistantSegment(input.turnId, input.segmentStartLength);
    replaceStreamingAssistantText({
      messageId: activeAssistantMessageId,
      turnId: input.turnId,
      text: appendStreamingAssistantPreviewText(input.turnId, input.deltaText),
    });
  };
  ...
  const commitStreamingAssistantSegment = (turnId: string, at: string, textOverride?: string) => {
    ...
    const fullText = textOverride ?? materializeRawAssistantDeltaText(turnId);
    const segmentStartLength = activeAssistantSegmentStartLengthRef.current.get(turnId) ?? 0;
    const text = textOverride ?? (fullText ? fullText.slice(segmentStartLength) : streamingAssistant.text);
    if (text) {
      dispatchSurfaceEvent({
        type: "message.appended",
        at,
      ...
      emittedAssistantTextRef.current.set(turnId, fullText);
      setRawAssistantDeltaText(turnId, fullText);
  ...
  const closeAssistantSegment = (turnId: string, at = new Date().toISOString()) => {
    ...
    activeAssistantSegmentStartLengthRef.current.delete(turnId);
  };
  ...
  const resetAssistantTurnState = (turnId: string) => {
    ...
    deleteRawAssistantDeltaText(turnId);
  };
  ...
  const resetSwitchRuntimeState = () => {
    ...
    activeAssistantSegmentStartLengthRef.current.clear();
    activeAssistantSegmentPreviewTextRef.current.clear();
    emittedAssistantTextRef.current.clear();
    replaceStreamingAssistantText(null);
    clearRawAssistantDeltaText();
    turnUserTextRef.current.clear();
    toolFamilyStateRef.current.clear();
    toolPreviewStateRef.current.clear();
    pendingToolPreviewArgumentChunksRef.current.clear();
  ...
  const directRunStatusEnabled = shouldUseDirectTuiPinnedRunStatus({
    ...
    hasTransientRunStatusLine: Boolean(runIndicator) && !hasActiveToolSummary,
    isTty: Boolean(process.stdout.isTTY),
    hasActiveToolSummary,
    exitSummaryActive: exitSummaryDisplay !== null,
    rewindInFlight: Boolean(rewindInFlight),
  ...
  const runStatusAnimationActive = !directRunStatusEnabled && shouldAnimateDirectTuiRunStatus({
  ...
  const fastAnimationActive =
    ...
    || questionPanelActive;
  const shouldAnimate = fastAnimationActive || hasActiveToolSummary;
  const animationIntervalMs = fastAnimationActive ? ANIMATION_TICK_MS : TOOL_SUMMARY_ANIMATION_TICK_MS;
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setAnimationTick((previous) => previous + (fastAnimationActive ? ANIMATION_SPEED_MULTIPLIER : 1));
    }, animationIntervalMs);
    return () => {
      clearInterval(timer);
    };
  }, [animationIntervalMs, fastAnimationActive, shouldAnimate]);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setRunStatusAnimationTick((previous) => previous + ANIMATION_SPEED_MULTIPLIER);
  ...
  useEffect(() => {
    ...
    const sourceApplicationBackendPath = resolve(appRoot, "raxode-cli/backend/directApplicationBackend.ts");
    const distApplicationBackendPath = resolve(appRoot, "dist/raxode-cli/backend/directApplicationBackend.js");
    ...
    setBackendReadinessDigest(null);
    if (!useSourceApplicationBackend && !useDistApplicationBackend) {
      setRunIndicator(null);
      ...
      dispatchSurfaceEvent({
        ...
        message: createSurfaceMessage({
          ...
          kind: "error",
          createdAt: at,
          text: `backend spawn failed: missing ${sourceApplicationBackendPath} or ${distApplicationBackendPath}`,
        }),
      });
      return;
    }
    const backendCommand = process.execPath;
    const backendArgs = useSourceApplicationBackend
      ? ["--import", tsxLoader ?? "tsx", sourceApplicationBackendPath, "--ui=direct"]
      : [distApplicationBackendPath, "--ui=direct"];
    const child = spawn(
      backendCommand,
      backendArgs,
      {
        ...
        env: {
          ...
          TSX_TSCONFIG_PATH: process.env.TSX_TSCONFIG_PATH ?? resolve(appRoot, "tsconfig.json"),
    ...
    const flushChunk = (chunk: string, stream: "stdout" | "stderr") => {
      ...
      for (const rawLine of parts) {
        ...
        const readinessDigest = parseBackendReadinessDigestLine(line);
        if (readinessDigest) {
          setBackendReadinessDigest(readinessDigest);
          continue;
        }
        if (stream === "stderr") {
          const at = new Date().toISOString();
      ...
      try {
        ...
        while (!cancelled) {
          ...
          try {
            ...
            processedBytesThisTick += bytesRead;
          ...
          if (record.event === "agent_session_started" || record.event === "agent_session_completed") {
            ...
            const existing = persistedAgentRegistry.find((agent) => agent.agentId === childSessionId);
            const childName = typeof record.childName === "string" && record.childName.trim()
              ? record.childName.trim()
              : "agent";
            const childDescription = typeof record.childDescription === "string" && record.childDescription.trim()
              ? record.childDescription.trim()
              : undefined;
            const lifecycle = typeof record.childLifecycle === "string" && record.childLifecycle.trim()
              ? record.childLifecycle.trim()
              : "oneshot";
            const status = record.event === "agent_session_started" ? "active" : "idle";
            const workspace = typeof record.workspaceRoot === "string" && record.workspaceRoot.trim()
            ...
            saveDirectTuiAgent({
              ...
              kind: "task",
              status,
              summary: childDescription
                ? `${childDescription} (${lifecycle})`
                : `${record.event === "agent_session_started" ? "running" : "completed"} ${lifecycle} agent`,
              workspace,
              createdAt: existing?.createdAt ?? at,
              updatedAt: at,
              lastSessionId: childSessionId,
            }, currentCwd);
            setAgentRegistryRevision((previous) => previous + 1);
            continue;
          }
          ...
          if (record.event === "tool_call_preview") {
            ...
            if (record.status !== "done") {
              appendToolPreviewArgumentChunk(stateKey, record.argumentsDelta);
            }
            const nextPreviewBase = {
              familyKey,
              familyTitle,
              ...
              argumentsPreview: previousPreview?.argumentsPreview ?? "",
            ...
            if (!shouldRenderPreview) {
              toolPreviewStateRef.current.set(stateKey, nextPreviewBase);
              continue;
            }
            const argumentsPreview = record.status === "done" && typeof record.arguments === "string"
              ? record.arguments
              : materializeToolPreviewArguments(stateKey, nextPreviewBase.argumentsPreview);
            const nextPreview = {
              ...nextPreviewBase,
              argumentsPreview,
            };
          ...
          if (record.event === "turn_result") {
            ...
            if (answer) {
              ...
              setRawAssistantDeltaText(turnId, answer);
            }
            ...
            for (const key of [...pendingToolPreviewArgumentChunksRef.current.keys()]) {
              if (key.startsWith(`${turnId}:`)) {
                pendingToolPreviewArgumentChunksRef.current.delete(key);
              }
            }
          ...
          if (record.event === "assistant_delta" && typeof record.text === "string" && record.text.length > 0) {
            ...
            appendRawAssistantDeltaTextChunk(turnId, record.text);
          ...
          if (processedLogByteOffsetRef.current < nextSize) {
            await yieldToEventLoop();
            if (processedBytesThisTick < LOG_TAIL_MAX_BYTES_PER_TICK) {
              continue;
            }
          }
          break;
  ...
  const persistConfigFile = (mutator: (draft: RaxodeConfigFile) => void): RaxodeConfigFile | null => {
    if (!configFile) {
      appendInlineError("Raxode config is unavailable. Please check ~/.raxode/config.json.");
      setSlashPanelNotice({
        tone: "danger",
  ...
  useEffect(() => {
    ...
    const animationTimer = setInterval(() => {
      setExitSummaryDisplay((current) => {
        ...
        return {
          ...current,
          animationStep: current.animationStep + ANIMATION_SPEED_MULTIPLIER,
        };
      });
  ...
  const scheduleTextSelectionAutoScroll = (event: { x: number; y: number }) => {
    const selection = textSelectionRef.current;
  ...
  const effectiveToolSummaryAnimationFrame = hasActiveToolSummary ? Math.floor(animationTick) : 0;
  ...
  const contextWindowSize = displayContextSnapshot?.compressionLimitTokens
    ?? displayContextSnapshot?.usableInputTokens
  ...
  const statusContextUsed = useMemo(
    ...
    [displayContextSnapshot?.sessionContextTokens, displayContextSnapshot?.lastRequestTotalTokens, displayContextSnapshot?.lastRequestInputTokens, displayContextSnapshot?.promptTokens],
  );
  const statusContextUsageLine = useMemo(
    () => formatStatusContextUsageLineFromSnapshot({
      snapshot: displayContextSnapshot,
      used: statusContextUsed,
      total: contextWindowSize,
    }),
    [displayContextSnapshot?.promptPackTokens, statusContextUsed, contextWindowSize],
  );
  ...
  const estimatedContextUsed = useMemo(
    ...
    [displayContextSnapshot?.sessionContextTokens, displayContextSnapshot?.lastRequestTotalTokens, displayContextSnapshot?.lastRequestInputTokens, displayContextSnapshot?.promptTokens, draftContextTokens],
  );
  ...
  const workspacePopupItems = useMemo(
    () => workspacePickerInputState && workspaceIndexSnapshot
      ? searchWorkspaceDirectories(workspaceIndexSnapshot, workspacePickerInputState.value.trim(), COMPOSER_POPUP_PAGE_SIZE * 4)
