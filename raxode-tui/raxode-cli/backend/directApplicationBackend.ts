/*
 * 文件定位：raxode-cli/backend direct TUI adapter。
 * 核心目的：让 direct TUI 的现有 UI/输入协议接入新的 applicationLayer Raxode 后端。
 */
...
type DirectHumanGateDecisionEnvelope = {
...
function resolveDirectApplicationMode(
  explicitMode: PraxisApplicationRuntimeMode | undefined,
): PraxisApplicationRuntimeMode {
  if (explicitMode) return explicitMode;
  return (process.env.RAXODE_DIRECT_APPLICATION_MODE ?? process.env.RAXODE_LEGACY_APPLICATION_MODE) === "dry-run"
    ? "dry-run"
    : "live";
}
...
function normalizeDirectPayload(raw: string): {
  ...
  try {
    ...
    const pastedAttachments: PraxisApplicationAttachment[] = (parsed.pastedContents ?? []).flatMap((entry, index) => {
      ...
      return [{
        id: typeof entry.id === "string" ? entry.id : `direct-paste:${index + 1}`,
        kind: "text" as const,
        tokenText,
    ...
    const fileAttachments: PraxisApplicationAttachment[] = (parsed.fileRefs ?? []).flatMap((entry, index) => {
      ...
      if (!localPath) return [];
      return [{
        id: typeof entry.id === "string" ? entry.id : `direct-file:${index + 1}`,
        kind: "file" as const,
    ...
    const imageAttachments: PraxisApplicationAttachment[] = (parsed.attachments ?? []).flatMap((entry, index) => {
      ...
      return [{
        id: typeof entry.id === "string" ? entry.id : `direct-attachment:${index + 1}`,
        kind: "image" as const,
...
function parseDirectHumanGateDecision(raw: string): DirectHumanGateDecisionEnvelope | undefined {
  const trimmed = raw.trim();
...
function inferRuntimeApprovalFeature(envelope: RuntimeApprovalEnvelope): {
  ...
  if (toolId?.startsWith("computer.") || toolId?.startsWith("computeruse.")) {
...
function approvalDecisionToRuntimeResolution(input: {
  decision: DirectHumanGateDecisionEnvelope;
...
type DirectContextSnapshot = ReturnType<typeof buildContextSnapshot>;
...
function buildContextSnapshot(result?: PraxisApplicationCommandResult) {
  ...
  const sessionContextTokens = typeof context?.sessionContextTokens === "number" && Number.isFinite(context.sessionContextTokens)
    ? context.sessionContextTokens
    : context?.historyEstimatedTokens ?? activeTokens;
  const compressionLimitTokens = typeof context?.compressionLimitTokens === "number" && Number.isFinite(context.compressionLimitTokens)
    ? context.compressionLimitTokens
    : undefined;
  const promptPackTokens = typeof context?.promptPackTokens === "number" && Number.isFinite(context.promptPackTokens)
    ? context.promptPackTokens
    : undefined;
...
function isProviderBackedContext(context: DirectContextSnapshot | undefined): boolean {
  if (!context) {
    return false;
  }
...
function contextFor(
  ...
  options: { lastProviderContext?: DirectContextSnapshot } = {},
) {
...
function directResultErrorCode(error: { code: string; message: string }): string {
...
function directStreamFrameMs(): number {
...
function directToolStageRecord(input: {
  ...
  return {
    ...
    familyTitle: stringMetadata(metadata, "familyTitle"),
    inputSummary: stringMetadata(metadata, "inputSummary"),
    argumentsPreview: stringMetadata(metadata, "argumentsPreview"),
...
function directModelStageRecord(input: {
  applicationEvent: PraxisApplicationEvent;
  sessionId: string;
  turnIndex: number;
}): Record<string, unknown> | undefined {
...
function directToolCallPreviewRecord(input: {
  applicationEvent: PraxisApplicationEvent;
  sessionId: string;
  turnIndex: number;
}): Record<string, unknown> | undefined {
...
function directMultiagentRecord(input: {
  applicationEvent: PraxisApplicationEvent;
  sessionId: string;
  turnIndex: number;
}): Record<string, unknown> | undefined {
  if (input.applicationEvent.kind !== "runtime") return undefined;
  const metadata = input.applicationEvent.metadata;
  const childSessionId = stringMetadata(metadata, "childSessionId");
  if (!childSessionId) return undefined;
  const eventId = input.applicationEvent.eventId;
  const isSpawned = eventId.endsWith(".multiagent.spawned");
  const isCompleted = eventId.endsWith(".multiagent.completed");
  if (!isSpawned && !isCompleted) return undefined;
  return {
    ts: input.applicationEvent.createdAt,
    event: isSpawned ? "agent_session_started" : "agent_session_completed",
    sessionId: input.sessionId,
    turnIndex: input.turnIndex,
    childSessionId,
    childAgentId: stringMetadata(metadata, "childAgentId"),
    childName: stringMetadata(metadata, "childName") ?? input.applicationEvent.message,
    childDescription: stringMetadata(metadata, "childDescription"),
    childLifecycle: stringMetadata(metadata, "childLifecycle"),
    initialMessageId: stringMetadata(metadata, "initialMessageId"),
    workspaceRoot: stringMetadata(metadata, "workingDirectory"),
    text: input.applicationEvent.message,
  };
}

function persistDirectMultiagentSession(input: {
  record: Record<string, unknown>;
  cwd: string;
  route: string;
  model: string;
}): void {
  const childSessionId = typeof input.record.childSessionId === "string" && input.record.childSessionId.trim()
    ? input.record.childSessionId.trim()
    : undefined;
  if (!childSessionId) return;
  const now = typeof input.record.ts === "string" && input.record.ts.trim()
    ? input.record.ts
    : new Date().toISOString();
  const existingAgent = listDirectTuiAgents(input.cwd).find((agent) => agent.agentId === childSessionId);
  const existingSession = loadDirectTuiSessionSnapshot(childSessionId, input.cwd);
  const childName = typeof input.record.childName === "string" && input.record.childName.trim()
    ? input.record.childName.trim()
    : existingAgent?.name ?? "agent";
  const childDescription = typeof input.record.childDescription === "string" && input.record.childDescription.trim()
    ? input.record.childDescription.trim()
    : undefined;
  const childLifecycle = typeof input.record.childLifecycle === "string" && input.record.childLifecycle.trim()
    ? input.record.childLifecycle.trim()
    : "persistent";
  const workspace = typeof input.record.workspaceRoot === "string" && input.record.workspaceRoot.trim()
    ? input.record.workspaceRoot.trim()
    : existingAgent?.workspace ?? existingSession?.workspace ?? input.cwd;
  const isStarted = input.record.event === "agent_session_started";
  const status = isStarted ? "active" : "idle";
  const summary = childDescription
    ? `${childDescription} (${childLifecycle})`
    : `${isStarted ? "running" : "completed"} ${childLifecycle} agent`;
  saveDirectTuiAgent({
    ...
    summary,
    workspace,
    createdAt: existingAgent?.createdAt ?? existingSession?.createdAt ?? now,
    updatedAt: now,
  ...
  saveDirectTuiSessionSnapshot({
    ...
    agentId: childSessionId,
    name: existingSession?.name ?? childName,
    workspace,
    route: existingSession?.route || input.route,
    model: existingSession?.model || input.model,
    createdAt: existingSession?.createdAt ?? existingAgent?.createdAt ?? now,
    updatedAt: now,
    selectedAgentId: childSessionId,
    agents: existingSession?.agents ?? [],
    messages: existingSession?.messages ?? [{
      messageId: `${childSessionId}:lifecycle`,
      kind: "status",
      text: summary,
      createdAt: now,
      updatedAt: now,
      metadata: {
        source: "multiagent.lifecycle",
        childLifecycle,
        childSessionId,
      },
    }],
    usageLedger: existingSession?.usageLedger ?? [],
    exitSummary: existingSession?.exitSummary,
  }, input.cwd);
}
...
export async function startDirectApplicationBackend(options: DirectApplicationBackendOptions = {}): Promise<void> {
  ...
  const enqueueRuntimeEventLog = (record: Record<string, unknown>) => {
    runtimeEventLogQueue = runtimeEventLogQueue.then(() => writeLog(logPath, record)).catch((error: unknown) => {
      errorOutput.write(`direct application backend runtime event log failed: ${error instanceof Error ? error.message : String(error)}\n`);
    });
  ...
  const resolveHumanGateDecisionPayload = (decision: DirectHumanGateDecisionEnvelope): boolean => {
  ...
  const enqueuePayload = (rawPayload: string) => {
    ...
    payloadQueue = payloadQueue.then(() => handler(rawPayload)).catch((error: unknown) => {
      errorOutput.write(`direct application backend payload failed: ${error instanceof Error ? error.message : String(error)}\n`);
    });
  };
  ...
  input.on("data", (chunk: string) => {
    ...
    for (const part of parts) {
      const decision = parseDirectHumanGateDecision(part);
  ...
  const [
    ...
    import("./application/runtimeReadiness.js"),
  ]);
  ...
  if (!created.ok) {
    ...
    errorOutput.write(`direct application backend failed: ${created.error.message}\n`);
    return;
  }
  ...
  const directTurnIndexByApplicationTurnId = new Map<string, number>();
  ...
  let activeDirectTurnIndex: number | undefined;
  let lastProviderContext: DirectContextSnapshot | undefined;
  transport.subscribe((applicationEvent) => {
    const directTurnIndex = (() => {
      const applicationTurnId = applicationEvent.turnId;
      if (applicationTurnId) {
        const existing = directTurnIndexByApplicationTurnId.get(applicationTurnId);
        if (existing !== undefined) {
          return existing;
        }
        if (activeDirectTurnIndex !== undefined) {
          directTurnIndexByApplicationTurnId.set(applicationTurnId, activeDirectTurnIndex);
          return activeDirectTurnIndex;
        }
      }
      return parseApplicationTurnIndex(applicationTurnId, turnIndex || 1);
    })();
    const modelStageRecord = directModelStageRecord({
      applicationEvent,
      sessionId,
      turnIndex: directTurnIndex,
    });
    if (modelStageRecord) {
      const modelContext = recordMetadata(modelStageRecord, "context") as DirectContextSnapshot | undefined;
    ...
    const toolStageRecord = directToolStageRecord({
      applicationEvent,
      sessionId,
      turnIndex: directTurnIndex,
    });
    ...
    const toolCallPreviewRecord = directToolCallPreviewRecord({
      applicationEvent,
      sessionId,
      turnIndex: directTurnIndex,
    });
    ...
    const multiagentRecord = directMultiagentRecord({
      applicationEvent,
      sessionId,
      turnIndex: directTurnIndex,
    });
    if (multiagentRecord) {
      persistDirectMultiagentSession({
        record: multiagentRecord,
        cwd,
        route: baseURL ?? providerRoute ?? provider ?? "(unconfigured)",
        model,
      });
      void enqueueRuntimeEventLog(multiagentRecord);
      return;
    }
    if (applicationEvent.kind !== "stream" || applicationEvent.message.length === 0) {
      return;
    }
    streamedTextByTurn.set(directTurnIndex, `${streamedTextByTurn.get(directTurnIndex) ?? ""}${applicationEvent.message}`);
    void enqueueRuntimeEventLog({
  ...
  const readiness = readinessModule.inspectRaxodeBackendReadinessWithLocalProbe({
    view: start.view,
  ...
  output.write(`backend readiness: ${JSON.stringify(readiness)}\n`);
  await writeLog(logPath, {
    ts: options.now?.() ?? new Date().toISOString(),
    event: "session_start",
    sessionId,
  ...
  await writeLog(logPath, {
    ts: readiness.generatedAt,
    event: "backend_readiness",
    sessionId,
    status: readiness.areas.some((area) => area.status === "missing")
      ? "missing"
      : readiness.areas.some((area) => area.status === "degraded")
        ? "degraded"
        : "ready",
    toolProfile: readiness.toolProfile,
    permissionProfile: readiness.permissionProfile,
    sandboxProfile: readiness.sandboxProfile,
    moduleInventory: readiness.moduleInventory,
    readiness,
  });
  ...
  const handlePayload = async (rawPayload: string) => {
    ...
    if (payload.startsWith("/rewind")) {
      ...
      directTurnIndexByApplicationTurnId.clear();
      await writeLog(logPath, {
        ts: options.now?.() ?? new Date().toISOString(),
    ...
    activeDirectTurnIndex = turnIndex;
    const result = await (async () => {
      try {
        ...
        activeDirectTurnIndex = undefined;
      }
    })();
    ...
    if (finalText.length > 0 && (streamedTextByTurn.get(turnIndex) ?? "").length === 0) {
      for (const chunk of chunkStreamText(finalText)) {
        ...
        await waitFrame(directStreamFrameMs());
      }
    }
    ...
    const resultMetadata = result.ok
      ? undefined
      : {
          errorCode: directResultErrorCode(result.view.error ?? result.error),
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  await startDirectApplicationBackend();
}
