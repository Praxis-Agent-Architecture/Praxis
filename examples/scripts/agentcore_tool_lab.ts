...
const logRoot = path.join(architectureRoot, ".devdoctor/runs/tool-lab", runId);
...
function normalizeMountedCodeBaseInput(
  ...
  if (tool === "code.search_Ripgrep") {
    ...
    input.directoryPath = firstNonBlankString(input.directoryPath, args.directory, args.path, args.cwd) ?? ".";
...
function buildToolAwarePrompt(agent: LabAgent, history: readonly ChatMessage[], userText: string): string {
  ...
  return [
    ...
    '{"tool_calls":[{"tool":"code.read","arguments":{"path":"package.json"}}]}',
    "网络搜索工具调用示例：",
